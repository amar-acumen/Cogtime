...
class My_ring_model extends Base_model 
{
 ...
 /******
    ...
    */
    ...
    {
        $res = $this->db->insert($this->db->RING ,$info);
   ...
   {
		//$this->db->where($where);
   		$this->db->update($this->db->RING_INV_USER,$arr,$where);
		 $this->db->last_query();
		if($is_msg	== 1)
	...
	{
		$this->db->delete($this->db->RING_INV_USER,$where); 
		if($is_msg	== 1)
			$this->db->update('messages',  array('i_ended'=>'1'),$msg_arr );
	}
	...
	public function get_list($where='',$i_start=null,$i_limit=null,$s_order_by='')
    {
        ...
        $sql    = " SELECT R.*,
					(SELECT s_category_name FROM {$this->db->RING_CAT} C WHERE C.id = R.i_category_id  ) AS s_category_name,

					(SELECT s_category_name FROM {$this->db->RING_CAT} C WHERE C.id = R.i_sub_category_id ) AS s_sub_category_name,
					
					 (SELECT count(*)
								FROM {$this->db->USERS} U, {$this->db->RING_INV_USER} RU
					...
					(SELECT count(*) FROM  cg_user_ring_post RP WHERE RP.i_ring_id = R.id  ) AS total_posts,
					
					(SELECT count(*) FROM  cg_user_ring_post RP , cg_user_ring_post_comments RPC 
    ...
    {
        $sql = "SELECT COUNT(*) total FROM {$this->db->RING_INV_USER} WHERE `i_joined`=1 AND i_ring_id={$ring_id}";
    ...
    {
        $sql = "DELETE FROM {$this->db->RING_INV_USER} WHERE id={$id}";
        $this->db->query($sql);
    }
	...
	{
		if($this->db->insert($this->db->RING_INV_USER ,$arr))
			return true;
		else	
			return true;
	}
	...
	public function new_get_ring_join_req_list($s_where,$i_start=null,$i_limit=null,$s_order_by=null)
    {
        try
        {
		  ...
		  rg.s_ring_name
							...
							LEFT JOIN cg_user_ring rg ON r.i_ring_id = rg.id
							WHERE r.i_invited_id=u.id AND r.i_request=1 AND r.i_joined=0 "
						.$s_where."";  #AND i_privacy=2
