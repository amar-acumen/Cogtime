   ...
   ### decline invitation 
    ...
    function request_already_sent($i_user_id='', $i_group_id = '')
    {
      	$SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s 
                        ...
                        $this->db->PRAYER_GROUP_MEMBERS, $i_user_id, $i_group_id);
	...
	public function get_my_groups_notificaions($i_user_id, $s_where, $i_start_limit='', $i_no_of_page='') {
		...
		if(count($result_arr)){
			...
			foreach($result_arr as $key=> $val){
				...
				if(($val['i_refferd_msg_id'] != '' || $val['i_refferd_msg_id'] != 0 ) 
					&& ($val['s_type'] == 'joining_req' || $val['s_type'] == 'invited')){
	...
	public function delete_prayer_group($id)
	{
	  ...
	  public function getTotalPrayerRoom($i_user_id){
		  ...
		  $SQL = sprintf("SELECT count(*) as count FROM %s WHERE i_user_id = %s  
		  								AND dt_end_time  <= '%s' ",
									   $this->db->CHAT_ROOM_INVITATION, $i_user_id, get_db_datetime());
