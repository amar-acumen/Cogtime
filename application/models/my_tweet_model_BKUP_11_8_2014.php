...
class My_tweet_model extends Base_model
{
	...
	public function get() {
		$sql = sprintf('SELECT * FROM '.$this->db->TWEETS.' order by id desc');
		...
		if("$i_start_limit" == "") {
			$sql = sprintf("
				  (SELECT  u.id i_user_id, 
					...
					(
						t.i_owner_id in 
						(SELECT u.id from %1\$stweets_followers c, %1\$susers u where c.s_status = 'accepted'
			 ...
			 $sql = sprintf("
				(SELECT  u.id i_user_id, 
					...
					(
						t.i_owner_id in 
						(SELECT u.id from %1\$stweets_followers c, %1\$susers u where 
