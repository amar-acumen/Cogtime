    ...
    public function authenticate($login_data)
     {
	  	...
	  	$PWDSQL = sprintf("SELECT s_password AS `s_password` FROM %s WHERE `id`='%s' ",
                        $this->db->ADMIN_USER, 1);
    ...
    */
	...
	function offline_this_user($user_id, $ip = '') {
		...
		/*}
			...
			echo '2 '.$delete_user = sprintf("delete from %susers_online 
							where i_user_id  = '%s' and s_ip = '%s'"
							, $this->db->dbprefix, $user_id, $ip
			);
		}
*/
		$this->db->query($delete_user);
   ...
   public function sign_up($info)
    ...
    {
        $SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE `s_email`='%s' ",
                        $this->db->USERS, $email_id);
    ...
    {
      	$SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE `s_first_name`='%s' ",
    ...
    {
      	$SQL = sprintf("SELECT id FROM %s WHERE `s_profile_url_suffix`='%s' ",
                        $this->db->USERS, $profile_url);
        $ROW = $this->db->query($SQL)->row_array(); #echo $this->db->last_query(); exit;
...
#echo nl2br($sql); #exit;
 ...
 ## ADDED TO MATCH PASSWORD
    ...
    {
        $SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE `id`='%s' AND `s_password` = '%s' ",
                        $this->db->USERS, $id, get_salted_password($pwd));
        $ROW = $this->db->query($SQL)->row_array();
    ...
    {
      	$SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE `s_first_name`='%s'  AND `s_last_name`='%s'",
                        $this->db->USERS, $user_first_name, $user_last_name);
        $ROW = $this->db->query($SQL)->row_array(); //echo $this->db->last_query(); exit;
