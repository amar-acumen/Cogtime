...
class Skill_model extends Base_model implements InfModel
{
  ...
  public function avg_rating($i_id)
  {
    ...
    */
    ...
    public function edit_skill_info($info, $user_id, $record_id)
    {
        try
        {
            $i_ret_=0;////Returns false
            if(!empty($info))
            {
                $c_ins=count($info);
                // check if user's data already exists
                //$is_exists = $this->check_skill_info($record_id);
                ...
                {
                    $i_ret_ =  $this->insert_skill($info);
                }
                else
                {
                       	$this->db->where('id', $record_id);
						$this->db->update($this->db->USER_SKILL, $info);
                        $i_ret_=$this->db->affected_rows();   
    ...
    public function check_skill_info($record_id)
    {
		$sql = sprintf("SELECT count(*) as total_count from %sUSER_EDUCATION where id  = '%s'"
							, $this->db->dbprefix, $record_id);
     ...
     public function insert_skill($arr=array()) {
		if(count($arr)==0) {
			return null;
		}
		//pr($arr);
		$this->db->insert($this->db->USER_SKILL, $arr);
    ...
    */
    public function delete_info($i_id)
    {
        try
        {
            ...
            if(intval($i_id)>0)
            {
                $s_qry="DELETE FROM ".$this->db->USER_SKILL." ";
                $s_qry.=" WHERE id=? ";
    ...
    public function delete_info_db($db_id)
    {
        try
        {
            ...
            if(intval($db_id)>0)
            {
                ...
                $q=sprintf("DELETE FROM %s WHERE id=%s",$this->db->USER_SKILL,$db_id);
