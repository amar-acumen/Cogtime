...
public function get_owner_post_by_id($i_user_id, $i_start_limit='', $i_no_of_page='')
{
if($i_start_limit == "") {
$sql="select distinct c.* from cg_users u ,cg_user_newsfeeds c where 1 and c.i_owner_id='".$i_user_id."'and u.i_isdeleted='1'";
}
else
{$sql="select distinct c.* from cg_users u ,cg_user_newsfeeds c where 1 and c.i_owner_id='".$i_user_id."'and u.i_isdeleted='1' limit ".$i_start_limit.",".$i_no_of_page;}
$query=$this->db->query($sql);
$result_arr=$query->result_array();
//echo $sql;exit;
//echo $i_start_limit;
...
public function get_total_owner_post_by_id($i_user_id)
{
$sql="select distinct c.* from cg_users u ,cg_user_newsfeeds c where 1 and c.i_owner_id='".$i_user_id."'and u.i_isdeleted='1'";
$query=$this->db->query($sql);
$result_arr=$query->num_rows();
return $result_arr;
}
}
