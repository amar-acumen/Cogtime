...
class Newsfeed_comments_model extends CI_Model {
    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null,$s_order_by=null)
    {
      ...
      try
      {
           ...
           $s_qry = "SELECT  A.*  "
                    ." FROM ". $this->db->USER_NEWSFEED_COMMENTS ." A " 
                    .$s_where; 
    ...
    */
	...
	public function insert($arr=array()) {
		...
		$this->db->insert('user_newsfeed_comments', $arr);
	...
	public function update($arr=array(), $id) {
		...
		$this->db->update('user_newsfeed_comments', $arr, array('id'=>$id));
	}
  ...
  public function postLikeUnlike($like_unlike_information_array,$like_or_unlike="like")
  {
		...
		// echo $like_or_unlike.' == '.$like_unlike_information_array['i_'.$like_or_unlike.'d_user_id'];
           ...
           $sql_present="SELECT COUNT(*) as total FROM {$this->db->dbprefix}user_newsfeed_{$like_or_unlike} WHERE i_newsfeed_id='{$like_unlike_information_array['i_newsfeed_id']}' 
       ...
       if($like_or_unlike=="like"){
           ...
           if( $is_present == 0){
            if($this->db->insert($this->db->dbprefix."user_newsfeed_like",$like_unlike_information_array))
            {

                 $sql_del ="DELETE FROM {$this->db->dbprefix}user_newsfeed_unlike WHERE i_newsfeed_id='{$like_unlike_information_array['i_newsfeed_id']}' AND i_unliked_user_id='{$like_unlike_information_array['i_liked_user_id']}'";
        ...
        if($is_present==0)
            {
            if($this->db->insert($this->db->dbprefix."user_newsfeed_unlike",$like_unlike_information_array))
                {

                    $sql_del ="DELETE FROM {$this->db->dbprefix}user_newsfeed_like WHERE i_newsfeed_id='{$like_unlike_information_array['i_newsfeed_id']}' AND i_liked_user_id='{$like_unlike_information_array['i_unliked_user_id']}'";
                    $this->db->query($sql_del);
                    $response['last_inserted_id'] = $this->db->insert_id();
                    $response['value'] = true ;
                    $response['message'] = "Success" ;
  ...
  public function get_people_liked_by_newsfeed_id($i_newsfeed_id, $i_start_limit="", $i_no_of_page="") {
		if("$i_start_limit" == "") {
...
#echo ($sql);
	...
	public function get_total_people_liked_by_newsfeed_id($i_newsfeed_id) {
		$sql = sprintf("SELECT count(*) count FROM %1\$suser_newsfeed_like c, %1\$susers u WHERE c.i_liked_user_id=u.id AND c.i_newsfeed_id = %2\$s order by c.dt_liked_on", $this->db->dbprefix, intval($i_newsfeed_id));
