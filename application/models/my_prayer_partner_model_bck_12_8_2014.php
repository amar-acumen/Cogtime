   ...
   /* add info for sending invitation mail */
    ...
    function friend_request_already_sent($i_requester_id='', $i_accepter_id = '')
    {
      	$SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE `i_requester_id`='%s'  AND `i_accepter_id` = '%s' AND `s_status` = 'pending'  ",
                        $this->db->USER_PRAYER_PARTNER, $i_requester_id, $i_accepter_id);
...
*/    }      
    ...
    */
    ...
    {
      	$SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE  (`i_requester_id`='%s'  OR `i_accepter_id` = '%s')  AND `s_status` = 'pending'",
                        $this->db->USER_PRAYER_PARTNER, $i_requester_id, $i_requester_id);
        $ROW = $this->db->query($SQL)->row_array(); #echo $this->db->last_query(); exit;
    ...
    function total_pending_prayer_partner_sent($i_requester_id='')
    {
      	$SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE (`i_requester_id`='%s' ) AND `s_status` = 'pending' ",
                        $this->db->USER_PRAYER_PARTNER, $i_requester_id);
        $ROW = $this->db->query($SQL)->row_array(); #echo $this->db->last_query(); exit;
    ...
    public function fetch_prayer_partner_points($arr=array())
    {
        //pr($arr);
         $qty = sprintf("SELECT 
						...
						ORDER BY p.id DESC",$this->db->PRAYER_PARTNER_POINTS,$arr['i_rec_user_id'], $arr['i_giving_user_id'],
						$arr['i_giving_user_id'], $arr['i_rec_user_id']);
