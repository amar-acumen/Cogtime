...
class Product_categories_model extends Base_model
{
	...
	public function get() {
		$sql = sprintf('SELECT * FROM '.$this->db->TRADE_CAT.' order by id desc');
	...
	public function get_by_id($id, $start_limit="", $no_of_page="") {
		if("$start_limit" == "") {
			$sql = sprintf('SELECT * FROM '.$this->db->TRADE_CAT.'  where id = %s',  $id);
		}
		else {
			$sql = sprintf('SELECT * FROM '.$this->db->TRADE_CAT.'  where id = %s limit %s, %s',  $id, $start_limit, $no_of_page);
		}
	...
	public function delete_by_id($id) {
		...
		## deleting trade cateory 
	     $sql = sprintf( 'DELETE FROM '.$this->db->TRADE_CAT.' WHERE id=%s', $id );
		 $this->db->query($sql);
