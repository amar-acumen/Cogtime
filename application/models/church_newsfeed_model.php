...
class Church_newsfeed_model extends Base_model
{
	...
	public function __construct() 
	{
		...
		public function get_newsfeeds_by_church_id($c_id,$user_id,$page,$offset)
		{
			...
			$sql='select cn.*,cp.privacy from cg_church_newsfeed cn left join cg_church_post_privacy_settings cp on cn.id=cp.feed_id where (privacy =1 and posted_by="'.$user_id.'") OR ( privacy=2) OR (privacy=3 and (viewed_by="'.$user_id.'" or posted_by="'.$user_id.'")) group by id order by id desc  limit '.$page.','.$offset.'';
		...
		{
			$sql='select cn.* from cg_church_newsfeed cn left join cg_church_post_privacy_settings cp on cn.id=cp.feed_id where (privacy =1 and posted_by="'.$user_id.'") OR ( privacy=2) OR (privacy=3 and (viewed_by="'.$user_id.'" or posted_by="'.$user_id.'")) group by id ';
			$query=$this->db->query($sql);
