...
class Ring_post_comments_model extends CI_Model {
    ...
    */
	...
	public function get_total_comments_by_ring_id($ring_id) {
		$sql = " SELECT count(*) as total_comment 
						 FROM cg_user_ring R , cg_user_ring_post RP , cg_user_ring_post_comments RPC
	...
	public function insert($arr=array()) {
		...
		$this->db->insert('cg_user_ring_post_comments', $arr);
		#echo $this->db->last_query();
  ...
  public function postLikeUnlike($like_unlike_information_array,$like_or_unlike="like")
  {
         ...
         $response['last_inserted_id'] = '';
		  ...
		  $sql_present="SELECT COUNT(*) as total FROM {$this->db->dbprefix}user_ring_post_{$like_or_unlike} 
       ...
       if($like_or_unlike=="like"){
           ...
           if( $is_present == 0){
            ...
            if($this->db->insert($this->db->dbprefix."user_ring_post_like",$like_unlike_information_array))
            {
...
//echo ($sql);
	...
	public function get_total_people_liked_by_ring_post_id($i_media_id ) {
			$sql = sprintf("SELECT count(*) count FROM %1\$suser_ring_post_like c, %1\$susers u 
							WHERE c.i_liked_user_id=u.id 
							AND c.i_ring_post_id = %2\$s 
	...
	public function get_owner_by_ring_post_id($post_id) {
		...
		$sql = sprintf('SELECT R.i_user_id as i_owner_id FROM cg_user_ring_post R, cg_user_ring_post_comments RC WHERE RC.i_ring_post_id = %s ',$post_id);
		$query = $this->db->query($sql);
		$result_arr = $query->result_array();
		return $result_arr[0];
	}
