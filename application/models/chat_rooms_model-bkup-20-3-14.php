...
class Chat_rooms_model extends Base_model
{
	...
	public function get() {
		$sql = sprintf('SELECT * FROM '.$this->db->CHAT_ROOM.' order by id desc');
	...
	public function get_by_id($id, $start_limit="", $no_of_page="") {
		if("$start_limit" == "") {
			$sql = sprintf('SELECT * FROM '.$this->db->CHAT_ROOM.'  where room_id = %s',  $id);
		}
		else {
			$sql = sprintf('SELECT * FROM '.$this->db->CHAT_ROOM.'  where room_id = %s limit %s, %s',  $id, $start_limit, $no_of_page);
		}
		$query = $this->db->query($sql);
		$result_arr = $query->result_array();
	...
	public function delete_by_id($id) {
		...
		## deleting ring cateory 
	     $sql = sprintf( 'DELETE FROM '.$this->db->CHAT_ROOM.' WHERE room_id=%s', $id );
		 $this->db->query($sql);
		#echo $this->db->last_query(); exit;
	}
	...
	public function get_list($where='',$i_start=null,$i_limit=null,$s_order_by='')
    {
        ...
        $sql    = " SELECT C.*,
						  ...
						  CAT.s_category_name 
					FROM room C 
					LEFT JOIN cg_room_cat RC ON C.room_id = RC.i_room_id 
					LEFT JOIN cg_chat_category CAT ON CAT.id = RC.i_cat_id 
	...
	public function change_status($status ,$id) {
		  ...
		  $sql = sprintf( "UPDATE {$this->db->CHAT_ROOM} SET `enable` = '%s'
						   WHERE `room_id` ='%s'"
					  , $status, $id );
	...
	/*public function my_chat_room_list($where='',$i_start=null,$i_limit=null,$s_order_by='')
    {
        ...
        $sql    = "	(SELECT 
						C.*, RC.i_cat_id,
						 RC.i_parent_cat_id, CAT.s_category_name ,
						 CR.i_user_id as owner_id,
						 'Y' is_owner
						FROM room C 
						LEFT JOIN cg_user_chat_rooms CR ON C.room_id = CR.i_room_id 
						LEFT JOIN cg_room_cat RC ON C.room_id = RC.i_room_id
						LEFT JOIN cg_chat_category CAT ON CAT.id = RC.i_cat_id 
					...
					(
							...
							C.*, RC.i_cat_id,
							 RC.i_parent_cat_id, CAT.s_category_name ,
							  ...
							  'N' is_owner
							FROM room C 
							LEFT JOIN cg_users_chat_room_invitation CR ON C.room_id = CR.i_room_id 
							LEFT JOIN cg_room_cat RC ON C.room_id = RC.i_room_id
							LEFT JOIN cg_chat_category CAT ON CAT.id = RC.i_cat_id 
							WHERE  C.enable = 1 AND C.member_only = 1 AND C.show_type = 0 
							AND CR.i_user_id in
							(     SELECT CI.i_user_id from room C1 , cg_users_chat_room_invitation CI 
	...
	*/
	...
	public function my_private_room_list($where='',$i_start=null,$i_limit=null,$s_order_by='')
    {
        ...
        $sql    = "	(SELECT 
					...
					( SELECT 
						...
						'Y' is_owner
						FROM room C 
						LEFT JOIN cg_prayer_grp_chat_room_invitation PR ON C.room_id = PR.i_chat_room_id 
						LEFT JOIN cg_prayer_group PG ON PG.id = PR.i_group_id
						WHERE C.enable = 1 AND C.member_only = 1 AND C.show_type = 0 
					...
					(
							...
							WHERE  C.enable = 1 AND C.member_only = 1 AND C.show_type = 0 
							AND PR.i_user_id in
							(     SELECT PR1.i_user_id from room C1 , cg_prayer_grp_chat_room_invitation PR1 
					...
					( 
						...
						'N' is_owner
						FROM room C 
						LEFT JOIN cg_ring_grp_chat_room_invitation RC ON C.room_id = RC.i_chat_room_id 
						WHERE C.enable = 1 AND C.member_only = 1 AND C.show_type = 0 
					...
					(
							...
							WHERE  C.enable = 1 AND C.member_only = 1 AND C.show_type = 0 
							AND RC.i_user_id in
							(     SELECT RC1.i_user_id from room C1 , cg_ring_grp_chat_room_invitation RC1 
	...
	public function my_ring_room_list($where='',$i_start=null,$i_limit=null,$s_order_by='')
    {
        ...
        $sql    = "	( SELECT 
						...
						UR.s_ring_name
						FROM room C 
						LEFT JOIN cg_user_ring UR ON C.room_id = UR.i_room_id 
						LEFT JOIN cg_ring_invited_user RC ON UR.id = RC.i_ring_id
					...
					(
							...
							(     SELECT  RC1.i_invited_id from room C1 
										  LEFT JOIN cg_user_ring UR1 ON C1.room_id = UR1.i_room_id 
										  LEFT JOIN cg_ring_invited_user RC1 ON UR1.id = RC1.i_ring_id
	...
	function getAllUserId_private_chat_room($room_id){
		...
		$SQL = "SELECT CI.i_user_id 
					FROM room C1 , cg_users_chat_room_invitation CI 
 ...
 {
	 $sql="SELECT i_room_id as id FROM ".$this->db->user_chat_rooms." WHERE id=".$id;
	 $query =$this->db->query($sql);
	 $result_arr = $query->result_array();
	 #pr($result_arr);
	 return $result_arr;
 ...
 {
	 $s_qry =  "SELECT * FROM "
					  .$this->db->chat_invitation. " WHERE i_chat_id =".$id;
