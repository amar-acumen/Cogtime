...
class Work_model extends Base_model implements InfModel
{
  ...
  public function avg_rating($i_id)
  {
    ...
    */
    ...
    public function edit_work_info($info, $user_id, $record_id)
    {
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                else
                {
                        ...
                        $this->db->update($this->db->USER_WORKS, $info);
                        $i_ret_=$this->db->affected_rows();   
 ...
 /*   // check education data exists or not
    public function check_work_info($record_id)
    {
...
*/   
     public function insert_work($arr=array()) {
        if(count($arr)==0) {
            return null;
        }
        //pr($arr);
        $this->db->insert($this->db->USER_WORKS, $arr);
