...
class Church_new_model extends Base_model
{
	...
	public function __construct() 
	{
		...
		function get_churchusers($c_id,$i_start=0,$order_by=' cm.id DESC',$i_limit=0){
    		$limit = (is_numeric($i_start) && is_numeric($i_limit)) ? " Limit " . intval($i_start) . "," . intval($i_limit) : '';
        	$order_by =  " ORDER BY {$order_by}" ;
		$sql ='select u.id, u.s_email, CONCAT(u.s_first_name, " ", u.s_last_name) AS user_name from cg_users u HERE u.id NOT IN (select member_id from cg_church_member where is_deleted=0 AND church_id = "'.$c_id.'" AND is_blocked = 1) '.$order_by.' '.$limit.' ';		//echo $sql;exit;
            $query = $this->db->query($sql);
            $result = $query->result();
            return $result;
