...
class Church_new_model extends Base_model
{
	...
	public function __construct() 
	{
		...
		$sql ='select *,u.id AS mid, CONCAT(u.s_first_name, " ", u.s_last_name) AS member_name,cm.id AS cmid from cg_church_member cm , cg_users u WHERE cm.member_id=u.id AND cm.church_id = "'.$c_id.'" AND cm.is_deleted=0 AND cm.is_leave = 0 '.$where.' '.$order_by.' '.$limit.' ';	//	echo $sql;
        ...
        function get_churchmembers_count($c_id,$where=''){
         ...
         //   WHERE cm.church_id = '.$c_id.' AND cm.is_deleted=0 AND cm.is_leave = 0'.$where;
				//echo $sql;
            $sql = 'select COUNT(*) AS totrow  from cg_church_member cm , cg_users u WHERE cm.member_id=u.id AND cm.church_id = "'.$c_id.'" AND cm.is_deleted=0 AND cm.is_leave = 0 '.$where;
            $query = $this->db->query($sql);
            $result = $query->result();
		...
		function get_churchusers($c_id,$i_start=0,$order_by='',$i_limit=0,$s_where){
    		$limit = (is_numeric($i_start) && is_numeric($i_limit)) ? " Limit " . intval($i_start) . "," . intval($i_limit) : '';
        	$order_by =  " ORDER BY {$order_by}" ;
		echo $sql ='select u.id, u.s_email, CONCAT(u.s_first_name, " ", u.s_last_name) AS user_name from cg_users u , cg_church r  WHERE u.id NOT IN (select member_id from cg_church_member cm where cm.is_deleted=0 AND cm.church_id = "'.$c_id.'" AND cm.is_leave = 0 AND cm.is_blocked = 1)'.$s_where.' GROUP BY u.s_email '.$order_by.' '.$limit.' ';		//exit;
            $query = $this->db->query($sql);
            $result = $query->result();
            return $result;
		...
		function get_churchusers_count($c_id,$s_where){
			$sql = 'select COUNT(*) AS totrow  from cg_users u , cg_church r WHERE u.id NOT IN (select member_id from cg_church_member where is_deleted=0 AND church_id = "'.$c_id.'" AND is_leave = 0 AND is_blocked = 1)'.$s_where.'  GROUP BY u.s_email ';
            $query = $this->db->query($sql);
            $result = $query->result();
            return $result[0]->totrow;
		}
	...
	public function delete_prayer_group($id) {
        $sql = 'DELETE FROM  cg_church_prayer_group WHERE id="'.$id.'"';
        $this->db->query($sql);

        $sql1 = 'DELETE FROM  cg_church_prayer_group_members WHERE i_prayer_group_id="'.$id.'"';
        $this->db->query($sql1);
        $sql2 = 'DELETE FROM  cg_church_prayer_group_notifications WHERE i_prayer_group_id="'.$id.'"';
        $this->db->query($sql2);
	...
	function get_church_invited_members($c_id,$i_start=0,$order_by='id DESC',$i_limit=0){
    		$limit = (is_numeric($i_start) && is_numeric($i_limit)) ? " Limit " . intval($i_start) . "," . intval($i_limit) : '';
        	$order_by =  " ORDER BY {$order_by}" ;
            $sql = 'select * from cg_church_member_invitation WHERE church_id = "'.$c_id.'" '.$order_by.' '.$limit.'';
				//echo $sql;
            $query = $this->db->query($sql);
            $result = $query->result();
            return $result;
