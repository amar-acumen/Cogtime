...
{$sql1="select p.s_photo , p.s_title , p.s_description , p.dt_created_on , a.s_name , a.id as albumid from cg_user_photos p ,cg_photo_album a ".$s_where." and p.i_user_id='".$i_user_id."'and p.i_status='1' and p.i_photo_album_id = a.id ORDER BY p.id DESC  limit ".$i_start_limit.",".$i_no_of_page."";}
...
function get_audio_post_by_id($i_user_id,$s_where,$i_start_limit='', $i_no_of_page=''){
   $sql = "select a.id as audioid , a.i_id_audio_album as audioalbumid ,a.s_title,a.s_artist,aab.s_name from cg_user_audio as a , cg_audio_album as aab ".$s_where." and a.i_user_id ='".$i_user_id."' and a.i_id_audio_album = aab.id and a.i_status = 1 ORDER BY a.id DESC limit ".$i_start_limit.",".$i_no_of_page." "; 
    $query=$this->db->query($sql);
$result_arr=$query->result_array();  
return $result_arr;  
}
