    ...
    */
	...
	function match_password($id='', $pwd ='')
    {
        $SQL = sprintf("SELECT COUNT(*) AS `check_count` FROM %s WHERE `id`='%s' AND `s_password` = '%s' ",
                        $this->db->ADMIN_USER, $id, get_salted_password($pwd));
        $ROW = $this->db->query($SQL)->row_array();
    ...
    {
        $SQL = sprintf("SELECT CONCAT(s_name,' ',s_last_name) AS `user_name` FROM %s WHERE `id`='%s' ",
                        $this->db->ADMIN_USER, $id);
        $ROW = $this->db->query($SQL)->row_array(); 
    ...
    {
        $SQL = sprintf("SELECT s_email FROM %s WHERE `id`='%s' ",
                        $this->db->ADMIN_USER, $id);
        $ROW = $this->db->query($SQL)->row_array(); 
       //echo $ROW['user_name']; exit;
        return $ROW['s_email'];
