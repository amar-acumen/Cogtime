...
class Base_controller extends CI_Controller 
{
           ...
           try
             {
                  if( $this->session->userdata('loggedin')  && $this->session->userdata('is_admin') != 1 
				  	 && $this->session->userdata('is_admin') != 2)
                    {
                       header('location:'.base_url().'my-wall.html');exit;
                   } 
   ...
   public function check_login($store_url_session = TRUE,$user_type , $arr_autorized_user_types = array()) 
    {
         try{
            ...
            if($this->session->userdata('loggedin')=='' 
                ...
                || (trim($user_type)!="" && intval($user_type)!=intval($this->session->userdata('user_type'))
                || (!empty($arr_autorized_user_types) && !in_array(intval($this->session->userdata('user_type')), $arr_autorized_user_types)) )
            ) 
            {
	...
	public function show_event_calendar($user_id='', $year='', $month='', $temp='',$displayslider=0) {
		...
		$s_where = 'AND i_active = 1 AND YEAR(`d_display_date`)= '. $year .' AND MONTH(`d_display_date`)= '. $month;
	...
	public function get_new_notifications()
	{
		...
		if((is_array($efreebie_request_accept) && count($efreebie_request_accept))){
		 ...
		 ###################################################
			...
			if( count($prayer_points_arr) != 0 
							  || count($prayer_partner_arr) != 0 
							  || count($netpal_arr) != 0 
							  || count($friend_arr) != 0 
							  || count($message_arr) !=0 
							  || count($netpal_declined_arr) != 0 
							  || count($netpal_accepted_arr) != 0  
							  || count($friend_declined_arr) != 0 
							  || count($friend_accepted_arr) != 0 
							  || count($prayer_partner_declined_arr) != 0 
							  || count($prayer_partner_accepted_arr) != 0 
							  || count($photo_comment) != 0
							  || count($video_comment) != 0
							  || count($audio_comment) != 0
							  || count($event_comment) != 0
							  || count($event_invitation) !=0
							  || count($tweet_comment) !=0
							  || count($ring_comment) !=0
							  || count($ring_post) != 0
							  || count($blog_comment) !=0
							  || count($retweet) != 0
							  || count($prayer_r_commit) !=0
							  || count($prayer_group_invitation) != 0
							  || count($prayer_group_deny_join_request) != 0
							  || count($prayer_group_accept_join_request) != 0
							  || count($prayer_group_chat_room_invitation) != 0
							  || count($prayer_group_joining) != 0
							  || count($etrade_request_declined) != 0
							  ||count($etrade_request_accept) != 0
							  ||count($etrade_request_recvd) != 0
							  ||count($eswap_request_recvd) != 0
							  ||count($eswap_request_accept) != 0
							  ||count($eswap_request_declined) != 0
							  ||count($efreebie_request_recvd) != 0
							  ||count($efreebie_request_declined) != 0
							  ||count($efreebie_request_accept) != 0
							  ||count($etrade_request_canceled_buyer) != 0
							  || count($user_chat_room_invitation) != 0
							  || count($emergency_prayer_click) != 0)
			{
 ...
 public function new_getImList_Ajax($page=0, $type='all', $s_name = ''){
		...
		#### new exclude arr ###
			...
			$exclude = array();
			if($ret_['i_isfriend'] == '0'){
				array_push($exclude,'friend');
			}
			if($ret_['i_isnetpal'] == '0'){
				array_push($exclude,'netpal');
			}
			if($ret_['i_isprayerpartner'] == '0'){
