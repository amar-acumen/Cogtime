...
class Prayer_group extends Base_controller
{
	...
	public function search_similar_prayer_group()
    {
			...
			$is_exists = $this->prayer_group_model->checkGroupNameExists(trim($this->input->post("txt_group_name")),  intval(decrypt($this->session->userdata('user_id'))));
           ...
           //pr($arr_messages);
            if( count($arr_messages)==0 ) {
                ...
                ### get list of similar group name and denomination
				 ...
				 $grp_name = trim($this->input->post('txt_group_name'));
   ...
   ### edit a post
	...
	//echo '1';
		...
		else
		{
			...
			$arr_ring=$this->input->post('ringid');
			#pr($arr_group);
			foreach($arr_ring as $val)
			{
				$arr_ringuser=$this->input->post('ringuser_'.$val);
			}
		$complete_arr_frnd =  array();
		$contact_arr = array();
		...
		$complete_arr_frnd = array_unique($complete_arr_frnd);
		$complete_arr_frnd = array_filter($complete_arr_frnd);
		insert_invitation($group_id,$_POST,$this->db->prayer_group_invitation,'i_group_id','prayer_group');
