...
class Events extends Base_controller {
    ...
    private $events_pagination_per_page = 3;
    ...
    public function event_detail($id) {
        try {
            ...
            $content = ob_get_contents();
            $content_obj = json_decode($content);
            $data['feedback_content'] = $content_obj->html;
    ...
    public function comments_ajax_pagination($i_media_id, $page = 0) {
        try {
            ...
            $result = $this->events_comments_model->get_by_event_id($i_media_id, $page, $this->comments_per_page);
    ...
    public function feedback_ajax_pagination($i_media_id, $page = 0) {
        try {
            ...
            $result = $this->events_feedback_model->get_feed_list($i_media_id, $page, $this->feedbacks_per_page);
            $resultCount = count($result);
...
//                'js/tab.js',
            ...
            $VIEW = "logged/my_events/find-events.phtml";
            parent::_render($data, $VIEW);
        } catch (Exception $err_obj) {
    ...
    public function search_events_ajax_pagination($page = 0) {
        try {
            $cur_page = intval($page) + intval($this->pagination_per_page);
            //$data = $this->data;
            //$order_by = " `dt_start_time` ASC ";
            ...
            if (isset($_POST['search_basic']) && $_POST['search_basic'] == 'Y') :
                ...
                $WHERE_COND = "and e.i_status =1 ";

                $s_name = get_formatted_string(trim($this->input->post('txt_name')));
                $WHERE_COND .= ($s_name == '') ? '' : ' AND e.s_title LIKE "%' . $s_name . '%" ';
...
//            $WHERE_COND .= ($s_phone == '') ? '' : ' AND C.s_phone LIKE "' . $s_phone . '%"';
                ...
                $WHERE_COND .= ($s_postcode == '') ? '' : ' AND e.s_postcode LIKE "' . $s_postcode . '%"';
            ...
            $result = $this->events_model->get_all_events($WHERE_COND, $page, $this->pagination_per_page);
            //pr($result,1);
            //echo $s_where;exit;
            $total_rows = $this->events_model->get_total_all_events($WHERE_COND);
            ...
            $VIEW_FILE = "logged/my_events/find_events_ajax.phtml";
