...
class Prayer_group extends Base_controller
{
    ...
    public function index() 
    {
        try
        {
            ...
            $VIEW = "logged/holy_place/prayer_group/prayer-group.phtml"; 
	...
	public function request() 
    {
        try
        {
			...
			$data['all_grp_arr'] = $this->prayer_group_model->get_other_groups_names_arr($i_user_id);
            ...
            $VIEW = "logged/holy_place/prayer_group/requset-prayer.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
	...
	public function get_pending_groups_requests_sent($page=0, $notif_grp_by = '-1')
	{
		...
		$data['prayer_grp_info'] = $this->prayer_group_model->get_pending_groups_requests_sent($user_id,$s_where); 
		$data['type'] = 'sent';
		
		
		$VIEW_FILE = "logged/holy_place/prayer_group/request_listing_ajax.phtml";
	...
	public function get_pending_groups_requests_recieved($page=0, $notif_grp_by = '-1')
	{
		...
		$data['prayer_grp_info_arr'] = $this->prayer_group_model->get_pending_groups_requests_recieved($user_id,$s_where); 
		$data['type'] = 'recv'; //pr($data['prayer_grp_info'],1);
	
		$VIEW_FILE = "logged/holy_place/prayer_group/recv_request_listing_ajax.phtml";
	...
	public function notifications_ajax_pagination($page=0, $notif_grp_by = '-1')
    {
		...
		$result = $this->prayer_group_model->get_my_groups_notificaions($user_id, $s_where, intval($page), $this->notification_per_page);
		$total_rows = $this->prayer_group_model->get_total_my_groups_notificaions($user_id, $s_where);
		...
		$VIEW_FILE = "logged/holy_place/prayer_group/ajax/notification_list_ajax.phtml";
	...
	public function groups_ajax_listing($page=0, $sort_order = 1, $grp_val = 'all')
	{
		
		$user_id = intval(decrypt($this->session->userdata('user_id')));
		$orderby = ($sort_order == 1)?'dt_created_on DESC':'s_group_name ASC';
		...
		$VIEW_FILE = "logged/holy_place/prayer_group/group_listing_ajax.phtml";
        ...
        try
        {
			...
			if($data['chat_room_id'] != ''){
				...
				if($data['is_exists']  == 'true'){	
					$data['chat_detail_arr'] = $this->chat_rooms_model->get_by_id($data['chat_room_id']);
					$data['chat_invited_members'] = $this->chat_rooms_model->membersJoining_PrayerMeet($data['chat_room_id'], $prayer_group_id);	
				}
			}
			else{
            ...
            $VIEW = "logged/holy_place/prayer_group/prayer-group-details.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
	...
	public function post_message($group_id){
			...
			$_html='';
			 ...
			 if($message!='')
		     {
				...
				$info['dt_created_on'] = get_db_datetime();
				$info['u_ip'] = $ip;
				$prayer_post_id = $this->prayer_group_model->insert_grp_post($info);
				...
				$grp_notification_arr['i_user_id'] = $i_user_id;
				...
				echo json_encode( array('success'=>'true', 'msg'=>"Posted successfully.",'html'=>$_html,'view_more'=>$view_more,'cur_page'=>$cur_page));
			}
			 else
			 {
   ...
   ### edit a post
    ...
    public function search_invite_friends($prayer_group_id) 
    {
        try
        {
			...
			$this->session->set_userdata('is_friend', '');
			$this->session->set_userdata('is_netpal', '');
			$this->session->set_userdata('is_ppartner', '');
            ...
            # view file...
            ob_start();
            $content = $this->generate_friend_search_listing_AJAX($prayer_group_id);
            $content = ob_get_contents();
            $content_obj = json_decode($content);
            ...
            $VIEW = "logged/holy_place/prayer_group/search-and-invite-friends.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
	...
	public function accept_req($grp_id, $uid, $id)
	{
	...
	public function decline_req($grp_id, $uid, $id)
	{
		...
		$msg_arr	= array();
		$msgarr	= array('id'=>$id);
		$res = $this->prayer_group_model->decline_invitation($where,$msg_arr);
		$sender_id = intval(decrypt($this->session->userdata('user_id')));
	...
	public function add_private_chat_room_invite() 
	{
		...
		try
        {
	        ...
	        $arr_messages = array();
					...
					$start = strtotime(trim($this->input->post('todo_strt_frm')));
					$stop = strtotime(trim($this->input->post('todo_end_frm')));
					$diff = ($stop - $start)/60;

					if($diff  > 60) 
					{
						$arr_messages['todo_end_frm'] = "* Maximum 30 minutes period is allowed.";
					}
				...
				//$this->email->cc('arif.zisu@gmail.com');
					...
					$this->email->send();
									...
									$chat_invitation['dt_end_time'] = $end_time;
	...
	public function search_prayer_group($prayer_group_id) 
    {
        try
        {
            ...
            $VIEW = "logged/holy_place/prayer_group/search-and-join-create-prayer-group.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
    ...
    public function generate_prayer_group_listing_AJAX($page=0)
    {
		...
		## seacrh conditions : filter ############
			 ...
			 if(isset($_POST['hd_submit']) && $_POST['hd_submit'] == 'Y' ) :
				...
				$WHERE_COND .= ($s_name=='')?'':" AND pg.s_group_name LIKE '%".$s_name."%' ";
				
				$sel_denomination = intval(decrypt(trim($this->input->post('sel_denomination'))));
				$WHERE_COND .= ($sel_denomination=='-1' || $sel_denomination=='0')?'':" AND ( pg.i_denomination_id  =  '".$sel_denomination."')";
		...
		$data['result_arr'] =  $this->prayer_group_model->group_list($s_where,$page,$this->pagination_per_page);
		$total_rows  =   $this->prayer_group_model->group_list_count($s_where);
		//pr($data['result_arr']);
		$data['pagination_per_page'] = $this->pagination_per_page;
		...
		$data['current_page_1'] = $cur_page;
         ...
         //--------- end check
        # loading the view-part...
        $AJAX_VIEW_FILE = 'logged/holy_place/prayer_group/ajax/prayer_grp_list_ajax.phtml';
	...
	function delete_prayer_group() {
        ...
        if (count($member_arr)) {
            foreach ($member_arr as $val) {
                $sender_id = $val['i_owner_id'];
                $receiver_id = $val['post_owner_user_id'];
                $this->social_notifications_message($sender_id, $receiver_id, 'prayer_group_delete', $groupid);
            }
        }
        $this->prayer_group_model->delete_prayer_group($groupid);
        ob_start();
        $this->groups_ajax_listing(0);
        $content = ob_get_contents();
        $content_obj = json_decode($content);
	...
	{			
		...
		$arr_pp=array();
		$arr_ringuser=array();
		$arr_frnd=$this->input->post('frnd_type1');
		if(empty($arr_frnd))
		{
			$arr_frnd=array(0=>'0');
			#pr($arr_frnd);
		...
		//$arr_pp=explode(',',$inv_frnds);
		$arr_ring=$this->input->post('frnd_type4');
		//$arr_ring=explode(',',$inv_ring);
		
			$i=0;
			foreach($arr_ring as $ring)
				{
					
					$arr=explode('_',$ring);
					$arr_ringuser[$i]=$arr['0'];
		...
		$contact_arr = array_merge($arr_frnd,$arr_netpal);
		//$contact_arr = array_merge($contact_arr,$arr_netpal);
		//pr($contact_arr);
		
		$complete_arr_frnd =  array_merge($contact_arr,$arr_pp);
		//pr($complete_arr_frnd);
		$complete_arr_frnd =  array_merge($complete_arr_frnd,$arr_ringuser);
		...
		foreach($invited as $invited_u)
		{
			$Inv[]=$invited_u['user_id'];
		}
		#pr($Inv,1);
		foreach($complete_arr_frnd as $user_id)
		{ 
	...
	$groupid	= $this->input->post('gr_id');
		...
		$this->prayer_group_model->leave_prayer_group($groupid,$user_id);
