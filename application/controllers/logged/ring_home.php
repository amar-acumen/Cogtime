...
class Ring_home extends Base_controller
{
    ...
    public function index($i_ring_id) 
    {
        try
        {
			...
			$wh = " AND i_joined = 0 ";
			$data['pending_join_req_arr'] = $this->my_ring_model->get_pending_join_req_arr($wh);
            ...
            # view file...
            $VIEW = "logged/ring/ring-home.phtml"; 
    ...
    public function all_ring_post_ajax_pagination($i_ring_id, $page=0)
    {
		...
		$result = $this->ring_post_model->get_all_ring_post_by_ring_id($i_ring_id,$s_where, intval($page), $this->pagination_per_page);
		...
		$ring_members_ajax	= $this->my_ring_model->get_all_ring_members_by_ring_id($i_ring_id,'','','');
		...
		$VIEW_FILE = "logged/ring/ajax_ring/ajax_ring_post_listing.phtml";
		...
		if(count($arr_messages) == 0 )
		    {
				...
				$i_ring_post_id = $this->ring_post_model->insert($arr);
				
				
				$total_posts = $this->ring_post_model->get_total_all_ring_post_by_ring_id($ring_id);
				...
				$ring_members	= $this->my_ring_model->get_all_ring_members_by_ring_id($ring_id,'','','');
				foreach($ring_members as $arr)
				{
					$arr_invited_id[] = $arr['i_invited_id'];
				}
