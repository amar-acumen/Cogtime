			...
			#### end search condition ###
			  ...
			  {  
				 $WHERE = " WHERE 
				  ...
				  $result = $this->my_prayer_partner_model->fetch_multi_online_friends($WHERE, intval($page),
             ...
             foreach($if_exists as $r)
             {
                 ...
                 if($counter == $count){
					 ...
					 if($r['i_giving_user_id'] == $info['i_giving_user_id'] ){
                        $des.='<li class="no-brdr" id="pp_'.$r['id'].'"><div class="content" style="word-wrap: break-word;"><div><strong>'.$r['s_first_name'].' <p class="date-time" style="margin-top:2px;float:right;">'.getShortDateWithTime($r['dt_created_on'], 9).'</p> </strong></div><div class="clr"></div> '.nl2br($r['s_desc']).'</div><a href="javascript:void(0);" class="right" onclick="delete_('.$r['id'].')"><img src="'.base_url().'images/icons/delete_small.png"/></a></li>';
					 }else{
                    	...
                    	if($r['i_giving_user_id'] == $info['i_giving_user_id'] ){
                        	$des.='<li id="pp_'.$r['id'].'"><div class="content" style="word-wrap: break-word;"><div><strong>'.$r['s_first_name'].' <p class="date-time" style="margin-top:2px;float:right;">'.getShortDateWithTime($r['dt_created_on'], 9).'</p> </strong></div><div class="clr"></div> '.nl2br($r['s_desc']).'</div><a href="javascript:void(0);" class="right" onclick="delete_('.$r['id'].')"><img src="'.base_url().'images/icons/delete_small.png"/></a></li>';
					    }
						else{
