...
class Tweet_home extends Base_controller {
    ...
    public function __construct() {
        try {
            ...
            $this->upload_path = BASEPATH . '../uploads/tweet_photos/';
            $this->upload_photo_path = BASEPATH . '../uploads/tweet_photos/';
            $this->upload_video_path = BASEPATH . '../uploads/tweet_videos/';
...
//            $total_my_tweets = $this->my_tweet_model->get_total_my_tweets($user_id);
			...
			//pr($arr['data']);
                ...
                $total_my_tweets = $this->my_tweet_model->get_total_my_tweets($user_id);
                echo json_encode(array('success' => 'true', 'msg' => "Tweet posted successfully.", 'html' => $_html, 'view_more' => $view_more, 'cur_page' => $cur_page, 'total_my_tweets' => $total_my_tweets,'i_id'=>$i_id));
            }
        } else {
    ...
    public function post_reply($tweet_id) {
        ...
        if ($message != '') {
			...
			if($tweet_info['i_owner_id'] != $user_id )
			{
			$email_opt = $this->user_alert_model->check_option_email_user_id($user_id);
						if($email_opt['e_tweet_comments_received'] == 'Y'){
						
						$mail_arr['i_requester_id'] =get_username_by_id( $user_id);
						$mail_arr['i_accepter_id'] =  get_username_by_id($tweet_info['i_owner_id']);
						$mail_arr['s_type'] = 'e_tweet_comments_received';
						$mail_id=get_useremail_by_id($tweet_info['i_owner_id']);
				...
				//$this->email->cc('arif.zisu@gmail.com');
					...
					$this->email->subject($mail_arr["i_requester_id"].' has replied on your tweet');
    ...
    public function retweet($posted_from, $tweet_id) {
		...
		if(count($tweet_data->photo))
		{
			foreach($tweet_data->photo as $val)
			{
			$json_data['photo'][]=$val;
			}
		}
		//pr($tweet_data->video);
		if(!empty($tweet_data->video) && count($tweet_data->video))
		{
			$json_data['video']['image_name']=$tweet_data->video->image_name;
			$json_data['video']['url']=$tweet_data->video->url;
		}
		...
		if($json_data['tweet_owner_id'] != $user_id )
			{
			$email_opt = $this->user_alert_model->check_option_email_user_id($user_id);
						if($email_opt['e_retweet'] == 'Y'){
						
						$mail_arr['i_requester_id'] =get_username_by_id( $user_id);
						$mail_arr['i_accepter_id'] =  get_username_by_id($json_data['tweet_owner_id']);
						$mail_arr['s_type'] = 'e_retweet';
						$mail_id=get_useremail_by_id($json_data['tweet_owner_id']);
						 $this->load->library('email');
						 $this->load->helper('html');
					$email_setting  = array('mailtype'=>'html','charset'  => 'utf-8',
                  'priority' => '1');
				...
				//$this->email->cc('arif.zisu@gmail.com');
					...
					$this->email->subject($mail_arr["i_requester_id"].' has retweeted your tweet.');
					$this->email->message("$body");
 ...
 public function get_video() {
        try {
            ...
            $media_info = $this->my_tweet_model->get_video_by_id($i_media_id);
