            ...
            if (1) {
                ...
                if ($message != '' || !empty($imagevalue) || $this->input->post('txt_video_url') != '' || $this->input->post('txt_audio_url') != '') {
					...
					if ($this->input->post('txt_audio_url') != '') {
                        ...
                        try {
                            $json_data['audio']= html_entity_decode(trim($this->input->post('txt_audio_url')));
                        } catch (Exception $e) {
	...
	public function post_comment($feed_id) {
        ...
        if ($message != '') {
			...
			$com_html='<div id="comment_content_div"'.$cid.'"><div class="txt_content01 comments-number-content"> 
					 			<a href="javascript:void(0);"><div class="pro_photo" style="background:url('.get_profile_image_of_user('thumb',$user_details['s_profile_photo'],$user_details['e_gender']).') no-repeat center;width:48px; height:48px;"></div></a>
