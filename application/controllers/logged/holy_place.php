...
class Holy_place extends Base_controller
{
	...
	public function generate_read_bible_AJAX($page=0)
    {
		...
		$bookmark_page	= ($page/25)+1;
		
		$bookmark_page_for_next	= $bookmark_page+1;
		if($page==0)
			$bookmark_page_for_prev	= 1;
		else
			$bookmark_page_for_prev	= $bookmark_page-1;
			
		$wh	= " AND i_page_bookmark ='".$bookmark_page."'";//" AND i_page_bookmark ='".$bookmark_page_for_prev."'";
		$wh1	= " AND i_page_bookmark ='".$bookmark_page_for_next."'";
		
		$data['pagebkmark']	= $this->holy_place_model->get_all_pagebkmark_arr_for_user($wh);
		/*$data['pagebkmark1']	= $this->holy_place_model->get_all_pagebkmark_arr_for_user($wh1);
		
		$data['bookmark_page_for_prev']	= $bookmark_page_for_prev;
		$data['bookmark_page_for_next']	= $bookmark_page_for_next;*/
		$data['bkmark']	= $bookmark_page;
		...
		$config['cur_tag_open'] = '<li class="num_tag_link current_tag_for_bible_paging">';
...
//                                          'css/dd.css') );
			...
			#pr($data['chapter_data']);
			
			$wh	= " AND c.i_book_id='".$bookdetail[0]['id']."' AND c.s_chapter='".$data['chapter_data'][0]['s_chapter']."'";
            $data['verse_data']	= $this->holy_place_model->get_bible($wh,0,1,'');
			$data['verseid']	= $data['verse_data'][0]['verseid'];
			$data['versepointer']	= $this->holy_place_model->get_array_of_verse_pointer($data['verseid']);
			# view file...
	...
	public function search()
	{
		...
		if(preg_match("/^[\w\s]+[\s]?[0-9]+[:][0-9]+/", $searchtxt ))
		{
			...
			if($verses == null || $versepointer == null)
			{
	...
	public function add_highlights()
	{
		...
		else
		{
			...
			$insert_arr['i_pos1']		= trim($this->input->post('startpos'));
			$insert_arr['i_pos2']		= trim($this->input->post('endpos'));
			...
			$wh	= " AND i_verses_id='".$insert_arr['i_verses_id']."' AND (('".$insert_arr['i_pos1']."'>= i_pos1 AND '".$insert_arr['i_pos1']."'<= i_pos2)
					...
					OR ('".$insert_arr['i_pos1']."'<= i_pos1 AND '".$insert_arr['i_pos2']."'>= i_pos2)) ";
					
			$all_hilits_by_vid	= $this->holy_place_model->get_hilits_for_pos($wh);
			#pr($all_hilits_by_vid,1);
			$flag	= 0;
			$hidarr	= array();
			$vid	= '';
	...
	public function add_bookmark()
	{
		...
		{
			$insert_arr['i_verses_id']	= $this->input->post('versesid');
			$insert_arr['i_page_bookmark']	= $this->input->post('pagebookmark');
...
//                                          'css/jquery.bxslider.css') );
			...
			//$data['five_fruits_arr'] = $this->bible_fruits_model->get_fruit_list();
			
           
		  // pr($data['five_fruits_arr']);
   
            $VIEW = "logged/holy_place/new_5day.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
...
//                                          'css/dd.css') );
			...
			ob_start();
			$this->getAllBibleNotes($page);
			$content = ob_get_contents();
			$content_obj = json_decode($content);
			$data['BibleNotelisting'] = $content_obj->html;
			ob_end_clean();
	...
	public function update_reading_plan_by_day()
	{
		...
		$plan_type =  $this->holy_place_model->get_reading_plan_type($i_profile_id);
		
		$todays_plan_arr = $this->holy_place_model->get_tdays_reading_plan_info();
		$today_reading_html = '';
		
		
		
		if(count($todays_plan_arr)){
			
			$today_reading_html = '<h2>Day'.$todays_plan_arr[0]['i_day'].'</h2>';
			if($plan_type != 'Beginning To End'){
																		  
				$today_reading_html .= 		'<p> 
						 <span class="start" onclick="gotoVerse('.$todays_plan_arr[0]['i_verse_no_from'].')" style="cursor:pointer;">'.$todays_plan_arr[0]['s_book_name_from'].$todays_plan_arr[0]['s_chapter_from'].':'.$todays_plan_arr[0]['i_verse_from'].'</span> - <span onclick="gotoVerse('.$todays_plan_arr[0]['i_verse_no_till'].')" style="cursor:pointer;">'.$todays_plan_arr[0]['s_book_name_till'].$todays_plan_arr[0]['s_chapter_till'].':'.$todays_plan_arr[0]['i_verse_till'].'</span>
						  <br/>
						  <span class="start" onclick="gotoVerse('.$todays_plan_arr[0]['i_verse_no_from_n'].')" style="cursor:pointer;">'.$todays_plan_arr[0]['s_book_name_from_n'].$todays_plan_arr[0]['s_chapter_from_n'].':'.$todays_plan_arr[0]['i_verse_from_n'].'</span> - <span onclick="gotoVerse('.$todays_plan_arr[0]['i_verse_no_till_n'].')" style="cursor:pointer;">'.$todays_plan_arr[0]['s_book_name_till_n'].$todays_plan_arr[0]['s_chapter_till_n'].':'.$todays_plan_arr[0]['i_verse_till_n'].'</span>
						  </p>';
			  } 
			  else{
				  $today_reading_html .= ' <p> <span class="start" onclick="gotoVerse('.$todays_plan_arr[0]['i_verse_no_from'].')" style="cursor:pointer;">'.$todays_plan_arr[0]['s_book_name_from'].$todays_plan_arr[0]['s_chapter_from'].':'.$todays_plan_arr[0]['i_verse_from'].'</span> - <span onclick="gotoVerse('.$todays_plan_arr[0]['i_verse_no_till'].')" style="cursor:pointer;">'.$todays_plan_arr[0]['s_book_name_till'].$todays_plan_arr[0]['s_chapter_till'].':'.$todays_plan_arr[0]['i_verse_till'].'</span>
																			</p>';
			  }
		}
		else
		{
			$today_reading_html = '<p>Plan Not Started Yet</p>';
		}
		
		
		$progress_bar_data = get_bible_progress_bar($plan_type);
		
		$last_read_date = $this->holy_place_model->get_reading_plan_last_read_date();
		$last_read_date = getShortDate($last_read_date,5);
		...
		echo json_encode(array( "data"=>'Bible reading plan has been successfully updated',
								...
								"today_reading_html"=>$today_reading_html,
	...
	public function utility()
	{
			...
			if(count($data['readingplan']) > 0)
			{
				...
				$data['read_bible_data']	= $this->db->query($sql)->result_array();
				 ...
				 $data['todays_plan_arr'] = $this->holy_place_model->get_tdays_reading_plan_info();
	...
	public function getAllBibleNotes($id='')
	{
		$wh	= "";
		$cat	= $this->input->post('cat');
		if($cat!='')
		...
		$data['note_arr']	= $this->holy_place_model->get_note_bkmark_hilits('note',$wh);
		$data['category_html']	= makeOptionBibleCategory();
		$AJAX_VIEW_FILE = 'logged/holy_place/ajax_holy_place/new_ajax_bible_notes_library.phtml';
        
		if(count($data['note_arr'])){
			$listingContent = $this->load->view($AJAX_VIEW_FILE, $data, true);
		}
		else
			$listingContent = '<div class="nw-inside-grey-container" style="height: 25px;"><div class="nw-contents" style="float:left;">No results. </div></div>';
	...
	public function getAllHighilights($showeditdelete='', $id='')
	{#echo $id;exit;
		$wh	= "";
		$cat	= $this->input->post('cat');
		if($cat!='')
	...
	public function getOrganizerNotes(){
		
		$this->load->model('organizer_note_model');
		$i_profile_id = intval(decrypt($this->session->userdata('user_id')));
		$s_where =  " AND i_active =1  ";
		$data['note_arr'] = $this->organizer_note_model->get_by_user_id($i_profile_id,$s_where);
		
		$AJAX_VIEW_FILE = 'logged/holy_place/ajax_holy_place/new_ajax_personal_notes_library.phtml';
        $listingContent = $this->load->view($AJAX_VIEW_FILE, $data, true);
	...
	public function getAllCategory(){
		    ...
		    $data['category']		= $this->holy_place_model->get_category_list();
			$AJAX_VIEW_FILE = 'logged/holy_place/ajax_holy_place/new_ajax_category_library.phtml';
        	$listingContent = $this->load->view($AJAX_VIEW_FILE, $data, true);
			echo json_encode(array( "data"=>"Category has been successfully listed.",
									"html"=>$listingContent,
									"success"=>true));
	...
	{
		$i_profile_id = intval(decrypt($this->session->userdata('user_id')));
		$q=$this->db->delete('cg_bible_bookmark', array('i_user_id' =>$i_profile_id));
		$qh=$this->db->delete('cg_bible_highlights', array('i_user_id' =>$i_profile_id));
		$qn=$this->db->delete('cg_bible_note', array('i_user_id' =>$i_profile_id));
		
		echo json_encode(array( "msg"=>"reset successfull!!",
									"success"=>true));
		exit;
	}
