...
class My_wall extends Base_controller {
    ...
    public function index($s_member_type = '') {
        try {
            ...
            //code for prayer partner qualification mail sending start
            $is_pr_prtnr_mail_sent = get_is_pr_prtnr_q_mail_sent($i_profile_id);
            if ($is_pr_prtnr_mail_sent == 0 || $is_pr_prtnr_mail_sent == '') {
                ...
                $attachmnt_file_path = '/uploads/prayer_partner_t_c_files/' . $attachmnt_file_name;
                if (($total_frnds >= $min_frnds) && ($total_rings >= $min_rings) && ($total_prayer_grps >= $min_prayer_grps) && ($total_commitmnts >= $min_pr_commtmnts) && ($current_time >= $six_months_later)) {
                    ...
                    $mail_info = $this->mail_contents_model->get_by_name("prayer_qualification_mail_content");
...
//                    exit();
                    send_mail($arr);
                    update_prayer_partner_mail_sent_status($i_profile_id);
                }
            }
            //code for prayer partner qualification mail sending end
            ...
            if ($is_netpal_mail_sent == 0 || $is_netpal_mail_sent == '') {
                ...
                $min_pr_commtmnts = $admin_set_q_params_arr['min_pr_comttmnts'];
                $attachmnt_file_name = $admin_set_q_params_arr['t_c_file_name'];
                $attachmnt_file_path = '/uploads/netpal_t_c_files/' . $attachmnt_file_name;
                if (($total_frnds >= $min_frnds) && ($total_rings >= $min_rings) && ($total_prayer_grps >= $min_prayer_grps) && ($total_commitmnts >= $min_pr_commtmnts) && ($current_time >= $six_months_later)) {
                    $user_data = get_user_data_by_id($i_profile_id);
                    $user_f_name = $user_data[0]['s_first_name'];
                    $user_l_name = $user_data[0] ['s_last_name'];
