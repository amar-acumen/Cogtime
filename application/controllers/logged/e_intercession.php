...
public function all_request_ajax_pagination($page=0)
    {
		...
		## seacrh conditions : filter ############
		  $WHERE_COND = 'WHERE 1 AND i.i_is_enable  = 1 AND i.e_request_type = "On Going" AND DATE(i.dt_end_date)>'.$curr_date;
				...
				$dt_end_date = $this->input->post('srch_date_end');
				  ...
				  if($WHERE_COND != ''){
					  $WHERE_COND .= ($dt_start_date=='' && $dt_end_date == '')?''
					  				:" AND  ( DATE_FORMAT(i.dt_start_date , '%Y-%m-%d %H:%i') BETWEEN
		...
		if($this->session->userdata('search_condition') != ''){
			...
			$s_where = 'WHERE 1 AND i.i_is_enable  = 1 AND i.e_request_type = "On Going" AND DATE(i.dt_end_date) >"'.$curr_date.'"';
		}
		//exit;
	 ...
	 public function archive_index() 
    {
        try
        {
			...
			$this->session->set_userdata('search_condition','');
			ob_start();
		    $this->all_archive_request_ajax_pagination(0);
			$content = ob_get_contents();
			$content_obj = json_decode($content);
			$data['prayer_req_ajax_content'] = $content_obj->html; 
			$data['no_of_result'] = $content_obj->no_of_result;
            ...
            $VIEW = "logged/holy_place/e-intercession/archive-view-search-eintercession.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
	...
	public function all_archive_request_ajax_pagination($page=0)
    {
		$curr_date = date('Y-m-d');
		//echo $page;
		## seacrh conditions : filter ############
		  $WHERE_COND = 'WHERE 1 AND i.i_is_enable  = 1 AND i.e_request_type = "On Going" AND DATE(dt_end_date)<'.$curr_date;
