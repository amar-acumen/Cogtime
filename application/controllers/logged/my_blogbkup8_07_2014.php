...
class My_blog extends Base_controller
{
   ...
   # pr($data['blogdata']);
	...
	public function add_blog()
	{
        ...
        {
            $arr    = array("s_title"=>$blog_title,"s_description"=>$blog_desc,"i_user_id"=>$this->i_profile_id);
   ...
   //pr($result);
	...
	public function comments_ajax_pagination($i_blog_post_id , $page=0) 
	{
		 try
		 {
			...
			$result = $this->my_blog_post_model->show_all_comments($wh ,$page,
	...
	public function add_blog_post_cmnts()
	{
		...
		//pr($arr);exit;
			...
			if($i_blog_post_comment_id)
			{
				...
				//pr($blog_detail_arr);
					 ...
					 $notificaion_opt = $this->user_alert_model->check_option_user_id($this->i_profile_id);	
					  //pr($notificaion_opt);
						...
						## insert noifications ####
						if(($notificaion_opt['e_ring_comments_received'] == 'Y') && ($blog_detail_arr['i_user_id'] != $this->i_profile_id)){
				...
				echo json_encode(array('result'=>'success','msg'=>$SUCCESS_MSG , 'html'=>$no_of_cmnts, 'blog'=>$postcmnts_postid));
				exit;
			}
			else
			{
