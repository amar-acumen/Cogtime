...
class User_alert_settings extends Base_controller
{
    ...
    public function index() 
    {
        try
        {
            ...
            # view file...
            $VIEW = "logged/user_notifications/user_alert_settings.phtml"; 
		...
		try
		{
		   ...
		   //pr($arr_messages);
			if( count($arr_messages)==0 ) {
				...
				$arr_alert_arr = $this->user_alert_model->get_by_user_id($i_logged_id);
				
				if(is_array($arr_alert_arr) && count($arr_alert_arr)){
					
					$info['dt_updated_on'] = get_db_datetime();
					$_ret = $this->user_alert_model->update($info , $i_logged_id);
					
				}
				else{
					
					$info['dt_created_on'] = get_db_datetime();
					$_ret = $this->user_alert_model->insert($info);
				}
									
				echo json_encode( array('success'=>true,'arr_messages'=>$arr_messages,'msg'=>'Alert settings updated Successfully.') );
			}
			else
			{
