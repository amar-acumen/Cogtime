...
class Mymessages extends Base_controller 
{
    ...
    public function index() 
    {
    	try
         {
			...
			$VIEW = "logged/message_box/my-msg-inbox.phtml";
      ...
      public function inbox_ajax_pagination($page=0) 
	  {
		 ...
		 try
		 {
			...
			$s_where = $this->session->userdata('search_condition');//"AND (CONCAT(u.s_first_name,' ',u.s_last_name) LIKE '%david%')";//
			$i_user_id = intval(decrypt($this->session->userdata('user_id')));
			
		    $result = $this->data_messages_model->get_by_receiver($i_user_id, intval($page), $this->pagination_per_page , $s_where);
			//echo $this->db->last_query();  exit;
	        $total_rows = $this->data_messages_model->get_total_by_receiver($i_user_id , $s_where);
		  # echo $this->db->last_query();
			//pr($result);
			
			$this->load->library('jquery_pagination');
			$config['base_url'] = base_url().'logged/mymessages/inbox_ajax_pagination/';
            ...
            $VIEW_FILE = "logged/message_box/msg_list_ajax.phtml";
	...
	public function get_message_body() {
		...
		$this->load->model('data_messages_model');
		$message = $this->data_messages_model->get_by_id_receiver($i_message_id, $i_user_id);
	...
	public function delete_messages() 
	{
		...
		{
		     $this->data_messages_model->delete_by_id_receiver($message_id, intval(decrypt($this->session->userdata('user_id'))));
		}
			
			$content = '';
			ob_start();
			$this->inbox_ajax_pagination($current_page);
			$content =  ob_get_contents();
			ob_end_clean();
	...
	public function get_unread_message()
	{
			...
			$total_unread_messages = $this->data_messages_model->get_total_by_receiver($i_user_id , $s_where); 
			echo json_encode(array('result'=>'success','unread_msg_count'=>$total_unread_messages));
			exit;
