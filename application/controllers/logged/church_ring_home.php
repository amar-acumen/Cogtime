...
class Church_ring_home extends Base_controller
{
	...
	private $ring_members_pagination_per_page = 1;
    public function __construct()
     {
        try
        {
			...
			$this->load->model('church_ring_post_comments_model');
...
//            parent::_add_css_arr( array('css/church.css') );
	...
	public function post_comment($feed_id) 
	{
		...
		if($message!='')
		    {
                    /***************get ring id********************************/
                    $query = $this->db->get_where('cg_church_ring_post', array('id' => $feed_id));
                        
                            foreach ($query->result() as $row)
                                {
                                  $ring_id = $row->i_ring_id;
                                }
                                //die('ok');
                    /**************************************************/
				...
				$arr['u_ip'] = $ip;
                                ...
                                $arr['i_ring_id']= $ring_id; 
				$this->church_ring_post_comments_model->insert($arr);
				...
				$total_comments = $this->church_ring_post_comments_model->get_total_comments_by_ring_id($i_ring_id);
                
                $_html = ''."Comments "." (".count_church_ring_comment_link($feed_id).")";
              ...
              if($like_or_unlike =="Like"){
               $like_unlike_information_array = array( "i_ring_post_id"=>$window_id,
              ...
              else if($like_or_unlike =="Unlike")
                {
                 ...
                 $status = 0;
                 $response = $this->church_ring_post_comments_model->postLikeUnlike($like_unlike_information_array,strtolower($like_or_unlike));
                ...
                $all_user_unliked = $dislike_val[0];
					...
					$_html = ''."Liked by "." (".count_church_post_like_link($window_id).")";
	...
	public function fetch_comment_on_ring_post($i_media_id='')
	{
		try
		  {
			 ...
			 $data = $this->data;  
			  ...
			  $VIEW = "logged/church/comments/my_ring_view_comments_lightbox.phtml";
	  ...
	  public function comments_ajax_pagination($i_media_id , $page=0) 
	  {
		 try
		 {
			...
			$config['base_url'] = base_url()."logged/church_ring_home/comments_ajax_pagination/{$i_media_id}";
			...
			$VIEW_FILE = "logged/church/comments/my_ring_view_comments_lightbox_ajax.phtml";
		...
		try
		  {
            ...
            parent::_add_css_arr( array(//'css/jquery-ui-1.8.2.custom.css',
			  ...
			  $data = $this->data; 
			  $html  = ''; 
			  $result = $this->church_ring_post_comments_model->get_by_ring_post_id($i_media_id);
                         // var_dump($result);
			//echo $i_media_id;
			//pr($result);
			  if(count($result)){
				  ...
				  foreach($result as $key=> $val){
					...
					$logged_user_id = intval(decrypt($this->session->userdata('user_id')));
                                  if($logged_user_id == $val['i_user_id'] || $_SESSION['charch_super_admin'] =='yes' ||  $_SESSION['subadmin_role'] == 2){
					 $html .= '<div class="txt_content01 comments-number-content" id="ring-cmnt'.$val['id'].'"> 
									...
									<div class="left-nw-wal">
										  ...
										  <p>'.$DESC.'</p>
											 <p class="read-more">Updated on: '.get_time_elapsed($val['dt_created_on']).'</p>
                                                                      ...
                                                                      <p class="del-ring-cmnt"><a href="javascript:void(0);" onclick="del_ring_commnt(\''.$val['id'].'\',\''.$val['i_ring_post_id'].'\')">Delete</a></p>
		...
		try
		  {
			  $data = $this->data; 
			  $html = ''; 
			  $result = $this->church_ring_post_comments_model->get_people_liked_by_ring_post_id($i_media_id);
			  ...
			  if(count($result)){
				  foreach($result as $key=> $val){
						...
						$profile_link = get_profile_url($val['i_liked_user_id'],$val['s_profile_name']);
...
//                                          'css/dd.css') );
	...
	public function new_decline_req($ringid, $uid, $id, $listype, $is_msg=0)
	{
        ...
        function delete_ring_post(){
            ...
            $this->db->delete('cg_church_ring_post', array('id' => $id)); 
        ...
        function edit_ring_post(){
            ...
            $data = array(
               's_post_title' => $txt_title,
               's_post_description' => $edit_des,
               'dt_updated_on' => get_db_datetime()
            );

$this->db->where('id', $id);
$this->db->update('cg_church_ring_post', $data); 
 echo json_encode(array('msg'=>'ok'));
        }
        function delete_ring_post_comment(){
            $id = $this->input->post('id'); 
            $post_id= $this->input->post('post_id');
            $this->db->delete('cg_church_ring_post_comments', array('id' => $id)); 
            /***************************************/
            $query = $this->db->query('select count(*) as total from cg_church_ring_post_comments where  i_ring_post_id = "'.$post_id.'"');
            $rs = $query->result();
           // pr($rs);
            $total_comment = $rs[0]->total;
            /*************************************/
            echo json_encode(array('msg'=>'ok' , 'cmnt' => $total_comment));
        }



    public function members($i_ring_id) 
    {
        try
        {
            ...
            ob_end_clean();
           	#pr($data['ring_members'],1);
             
            # view file...
            $VIEW = "logged/church/ring_members.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
    ...
    public function ring_members_ajax_pagination($i_ring_id, $page=0)
    {
        ...
        $result = $this->church_ring_model->get_all_ring_members_by_ring_id($i_ring_id, intval($page), $this->ring_members_pagination_per_page);

        $total_rows = $this->church_ring_model->get_total_all_ring_members_by_ring_id($i_ring_id);
        ...
        $VIEW_FILE = "logged/church/ajax_ring/ajax_ring_members_listing.phtml";
    ...
    function delete_member()
    {
		...
		$this->church_ring_model->delete_ring_member_by_id($id);
