...
//										  'css/flexslider.css') );
   ...
   # pr($data['blogdata']);
			...
			$data['cat6_article']	=	$this->landing_page_cms_model->get_all_audios(" where v.i_category_id=6 ",0,1,'');
...
//										  'css/flexslider.css') );
           ...
           $data['pagination_per_page'] = $this->pagination_per_page;
           $rst = $this->landing_page_cms_model->get_all_audios(" where v.id='".$id."'");
           $data['detail_content'] = $rst[0];
		   $data['ogimage'] = base_url() . 'uploads/media_center_article/' . getThumbName($data['detail_content']['s_image'], 'main');
		  	$data['cat5_article']	=	$this->landing_page_cms_model->get_all_audios(" where v.i_category_id=5 ",0,4,'');
			$data['cat6_article']	=	$this->landing_page_cms_model->get_all_audios(" where v.i_category_id=6 ",0,1,'');
           ...
           $this->session->set_userdata('category',$cat);
			...
			$data['comments']  = $this->NEW_fetch_comment_audio($id);
           ...
           $VIEW = "logged/media_center/audio-detail.phtml"; 
		   //$VIEW = "logged/media_center/audio-detail1.phtml"; 
            parent::_render($data, $VIEW);
        }
        catch(Exception $err_obj)
        {
    ...
    public function details1($id	,$cat)
    {
        try
        {
...
//================================= like n comments ====================================   
    ...
    public function comment_news()
    {
        ...
        if($info['s_contents']!='')
        {
            $res = $this->landing_page_cms_model->comment_audio($info);
            if($res)
            {
                $success = true;
                $msg = "Successfully updated!";
        ...
        $no_of_comments = $this->landing_page_cms_model->get_total_comment_by_audio_id($info['i_news_id']);
        $html  = $this->NEW_fetch_comment_audio($info['i_news_id']);  
    ...
    public function fetch_comment_on_news()
    {
      ...
      public function comments_ajax_pagination($news_id , $page=0) 
      {
         try
         {
            ...
            $data = $this->data;  
            $result = $this->landing_page_cms_model->get_total_comment_by_audio_id($news_id , $page,
...
//================================= end like n comments ====================================   
	...
	public function NEW_fetch_comment_audio($news_id='')
	{
		try
		  {
			  $data = $this->data; 
			  ...
			  $data['result'] = $this->landing_page_cms_model->get_total_comment_by_audio_id($news_id);
...
//										  'css/flexslider.css') );
   ...
   # pr($data['blogdata']);
			...
			$data['article']	=	$this->landing_page_cms_model->get_all_audios(" where v.i_category_id='".$id."'" ,'','','');
