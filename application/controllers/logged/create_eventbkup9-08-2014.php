...
class Create_event extends Base_controller {
    ...
    public function add_event_ajax($id) {
        try {
            ...
            if (count($arr_messages) == 0) {
				...
				$start_time = $this->input->post('date_to') . ' ' . $this->input->post('todo_strt_frm');

                $info["dt_start_time"] = $start_time;
				

               // $end_time = get_db_dateformat($this->input->post('date_end'), '/') . ' ' . $this->input->post('todo_end_frm');
			    $end_time = $this->input->post('date_end') . ' ' . $this->input->post('todo_end_frm');
    ...
    public function edit_event($i_event_id) {
        try {
            ...
            $data['event_info']["dt_start_date"] = getShortDate($data['event_info']["dt_start_time"], 3);
            $data['event_info']["dt_end_date"] = getShortDate($data['event_info']["dt_end_time"], 3);
