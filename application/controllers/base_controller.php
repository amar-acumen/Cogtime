...
class Base_controller extends CI_Controller {
	...
	try{
       ...
       $this->rssparser->set_cache_life(30);                       // Set cache life time in minutes
		...
		//$feeds[] = $val;
                ...
                //pr($feeds);
		$this->data['scrolling_news']=$feeds;//$feeds;
		}
		catch (Exception $err_obj) {
    ...
    protected function _set_left_panel_data($i_profile_id) {
		...
		$arr_profile_info['is_pr_partner_q_mail_sent']=$this->session->userdata('is_pr_partner_q_mail_sent');
                $arr_profile_info['s_profile_photo']=$this->session->userdata('s_profile_photo');
                 $arr_profile_info['e_gender']=$this->session->userdata('e_gender');
...
$this->db->update('cg_church', $data); 
    ...
    $query_churchmember = $this->db->query($sql_churchmember);
     ...
     function find_member_church_by_email(){
        ...
        $query =  $this->db->query('Select * from cg_church where s_name like "%'.$txt_name_church.'%" AND s_address like "%'.$txt_address_church.'%" or s_postcode like "%'.$txt_postcode_church.'%" AND ch_email != "" ');
              //echo $this->db->last_query();
            $result = $query->result();
             $data['result_arr'] = $result;
            $VIEW_FILE = 'church_ajax_adv.phtml';
            echo $this->load->view($VIEW_FILE, $data,true);
