...
class Efreebie extends Admin_base_Controller
{
    ...
    public function index() 
    {
        ...
        try
        {
            ...
            $VIEW_FILE = "admin/trade_center/efreebie.phtml";
        ...
        try
        {
		  ...
		  if(isset($_POST['search_basic']) && $_POST['search_basic'] == 'Y' ) :
			...
			## seacrh conditions : filter ############
		   	...
		   	$result = $this->e_freebie_model->get_efreebie_product_list($s_where,$page,$this->pagination_per_page,$order_by);
            $resultCount = count($result);
			$total_rows = $this->e_freebie_model->get_efreebie_product_list_count($s_where);
			
			if( ( !is_array($result) || !count($result) ) && $total_rows ) {
                $page = $page - $this->pagination_per_page;
                
                $result = $this->e_freebie_model->get_efreebie_product_list_count($s_where, $page, $this->pagination_per_page,$ORDER_BY);
            }
			## end seacrh conditions : filter ############

			#Jquery Pagination Starts
           	$this->load->library('jquery_pagination');
            $config['base_url'] = base_url()."admin/trade_center/efreebie/product_ajax_pagination";
          ...
          echo  $this->load->view('admin/trade_center/free_product_ajax_listing.phtml', $data,TRUE);
		 }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    public function request_ajax_pagination($page=0)
    {
        try
        {
			...
			$s_where = $this->session->userdata('search_condition');
			## seacrh conditions : filter ############
		   	$result = $this->e_freebie_model->get_all_efreebie_request_product_list($s_where,$page,$this->pagination_per_page,$order_by);
            $resultCount = count($result);
			#echo $this->db->last_query(); exit;
			$total_rows = $this->e_freebie_model->get_all_efreebie_request_product_list_count($s_where);
			...
			#Jquery Pagination Starts
           	$this->load->library('jquery_pagination');
            $config['base_url'] = base_url()."admin/trade_center/efreebie/request_ajax_pagination";
            $config['total_rows'] = $total_rows;
            $config['per_page'] = $this->pagination_per_page;
            $config['uri_segment'] = 5;
            $config['num_links'] = 9;
			...
			# loading the view-part...
          echo  $this->load->view('admin/trade_center/free_request_ajax_listing.phtml', $data,TRUE);
		 }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
	...
	function change_status()
    {
		...
		$whr_arr = array('id'=>$id);
        $this->e_freebie_model->update_efreebie_product($data, $whr_arr);
    ...
    {
        $id = $this->input->post('id');
        $this->e_freebie_model->delete_products_and_requests($id);
    ...
    function change_request_status()
    {
        ...
        $data['i_isenabled'] = 3 - $current_status;
		$whr_arr = array('id'=>$id);
        $this->e_freebie_model->update_efreebie_request($data, $whr_arr);
    ...
    {
        $id = $this->input->post('id');
        $this->e_freebie_model->delete_requests($id);
 ...
 function generate_sub_categroies(){
   ...
   function get_details(){
	...
	$html = $this->load->view('admin/trade_center/freebie_detail.phtml',$data,true);
