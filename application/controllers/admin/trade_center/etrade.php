...
class Etrade extends Admin_base_Controller
{
    ...
    public function index() 
    {
        ...
        try
        {
            ...
            $VIEW_FILE = "admin/trade_center/e_trade.phtml";
    ...
    public function product_ajax_pagination($page=0)
    {
        try
        {
		  ...
		  if(isset($_POST['search_basic']) && $_POST['search_basic'] == 'Y' ) :
				...
				if($this->input->post('txt_start_price') != '' && trim($this->input->post('txt_end_price')) != ''){
					$txt_start_price = $this->input->post('txt_start_price');
					$txt_end_price = $this->input->post('txt_end_price');
					$WHERE_COND .= ($txt_start_price=='')?'':" AND ( p.f_unit_price BETWEEN  '".$txt_start_price."'  AND '".$txt_end_price."' )";
				}
				else{
					if($this->input->post('txt_start_price') != ''){
						$txt_start_price = $this->input->post('txt_start_price');
						$WHERE_COND .= ($txt_start_price=='')?'':" AND ( p.f_unit_price ='".$txt_start_price."' )";
					}
					if(trim($this->input->post('txt_end_price')) != ''){
						$txt_end_price = ($this->input->post('txt_end_price'));
						$WHERE_COND .= ($txt_end_price=='')?'':" AND ( p.f_unit_price ='".$txt_end_price."' )";
					}
				}
			...
			//$order_by = "p.s_category_name ASC ";
		   	$result = $this->e_trade_model->get_etrade_product_list($s_where,$page,$this->pagination_per_page,$order_by);
            $resultCount = count($result);
			#echo $this->db->last_query(); exit;
			$total_rows = $this->e_trade_model->get_etrade_product_list_count($s_where);
...
//						pr($this->uri->total_segments());
            ...
            $config['base_url'] = base_url()."admin/trade_center/etrade/product_ajax_pagination";
          ...
          echo  $this->load->view('admin/trade_center/product_ajax_listing.phtml', $data,TRUE);
		 }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    public function request_ajax_pagination($page=0)
    {
        try
        {
			...
			$s_where = $this->session->userdata('search_condition');
			## seacrh conditions : filter ############
		   	$result = $this->e_trade_model->get_etradeProductReq_list($s_where,$page,$this->pagination_per_page,$order_by);
            $resultCount = count($result);
			#echo $this->db->last_query(); exit;
			$total_rows = $this->e_trade_model->get_etradeProductReq_list_count($s_where);
			...
			#Jquery Pagination Starts
           	$this->load->library('jquery_pagination');
            $config['base_url'] = base_url()."admin/trade_center/etrade/request_ajax_pagination";
            $config['total_rows'] = $total_rows;
            $config['per_page'] = $this->pagination_per_page;
            $config['uri_segment'] = 5;
            $config['num_links'] = 9;
			...
			# loading the view-part...
          echo  $this->load->view('admin/trade_center/request_ajax_listing.phtml', $data,TRUE);
		 }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
	...
	function change_status()
    {
		...
		$whr_arr = array('id'=>$id);
        $this->e_trade_model->update_etrade_product($data, $whr_arr);
    ...
    {
        $id = $this->input->post('id');
        $this->e_trade_model->delete_products_and_requests($id);
    ...
    function change_request_status()
    {
        ...
        $data['i_isenabled'] = 3 - $current_status;
		$whr_arr = array('id'=>$id);
        $this->e_trade_model->update_etrade_request($data, $whr_arr);
    ...
    {
        $id = $this->input->post('id');
        $this->e_trade_model->delete_requests($id);
 ...
 function get_details(){
	...
	$html = $this->load->view('admin/trade_center/etrade_detail.phtml',$data,true);
