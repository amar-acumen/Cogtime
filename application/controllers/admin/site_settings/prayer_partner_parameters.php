...
class Prayer_partner_parameters extends Admin_base_Controller {
    ...
    function index() {
        try {
            ...
            //echo BASEPATH;
//            $upload_path = BASEPATH . '../uploads/prayer_partner_t_c_files/';
            $result_arr = array();
    ...
    function update_prayer_partner_qualification_parameters() {
        ...
        if (!empty($file_name_arr)) {
            ...
            $upload_path = BASEPATH . '../uploads/prayer_partner_t_c_files/';
            //$this->upload_image = $upload_path. $file_name;
            $upload_image = $upload_path. $file_name;
            @move_uploaded_file($_FILES['min_intros']['tmp_name'], $upload_image);
        }
        if (empty($file_name)) {
            $file_name = $t_c_file;
        }
