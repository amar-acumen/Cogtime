...
class Advertisement extends Admin_base_Controller
{
    ...
    public function index() 
    {
        ...
        try
        {
            ...
            $VIEW_FILE = "admin/site_settings/advertisement/manage_advertisement.phtml";
    ...
    public function add_advertisement()
    {
		try
		{
			...
			#echo "file_error_$fileElementName" ;echo $data["file_error_$fileElementName"]; exit;
				...
				$re_page = admin_base_url() ."site_settings/advertisement.html";
		 	...
		 	// End Submitted Form
			 $VIEW_FILE = "admin/site_settings/advertisement/add-edit_advertisement.phtml";
            parent::_render($data, $VIEW_FILE);
    ...
    public function ajax_pagination($page=0)
    {
        try
        {
			...
			$total_rows = $this->mod_banner->gettotal_info($s_where);
            ...
            $config['base_url'] = base_url()."admin/site_settings/advertisement/ajax_pagination";
          ...
          echo  $this->load->view('admin/site_settings/advertisement/manage_advertisement_ajax.phtml', $data,TRUE);
		 }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
	 ...
	 public function change_status()
	 {
			...
			{
				$this->mod_banner->change_adv_status($i_status,$ID);
				if($i_status==1)
				   {
					 
						$action_txt =
							 '<input name="" title="ENABLED" type="button" class="btn-01" onclick="javascript:changeStatus(\''.$ID.'\',\'2\',\''.$i_status.'\')"  value="DISABLED"/>';
					
				   }
				 else if($i_status==2)
				   {
						$action_txt =
							 '<input name="" title="DISABLED" type="button" class="btn-01" onclick="javascript:changeStatus(\''.$ID.'\',\'1\',\''.$i_status.'\')"  value="ENABLED"/>';
