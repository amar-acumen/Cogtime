...
class Netpal_parameters extends Admin_base_Controller {
    ...
    function __construct() {
        try {
            ...
            $this->upload_path = BASEPATH . '../uploads/netpal_t_c_files/';
    ...
    function index() {
        try {
            ...
            $result_arr = array();
            $result_arr = $this->admins_user_model->get_netpal_q_params(1);
            //pr($result_arr);
            $data['result_arr'] = $result_arr[0];
            $view_file = "admin/site_settings/netpal_parameters.phtml";
    ...
    function update_netpal_qualification_parameters() {
        ...
        $update = $this->admins_user_model->update_netpal_q_params($arr, 1);
        $location = admin_base_url() . 'site_settings/netpal_parameters?update=yes';
        if (!empty($update)) {
