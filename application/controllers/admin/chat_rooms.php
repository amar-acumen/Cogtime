        ...
        try
        {
		 ...
		 $WHERE_COND = '';
		   if(isset($_POST['search_basic']) && $_POST['search_basic'] == 'Y' ) :
			  ...
			  $WHERE_COND .= ($txt_username=='')?'':" AND ( u.first_name LIKE '%".$txt_username."%'  
															OR  u.last_name LIKE '%".$txt_username."%'
		...
		$arr_messages = array();
		  ...
		  if($_POST){
				...
				if( count($arr_messages)==0 ) {
						  ...
						  $host = "127.0.0.1";
						  $port = 51127;
						  $apiCommand = '<?xml version="1.0" encoding="UTF-8"?><Command group="default" api_pwd="3874-3459-9293-2194" type="add_room" name="'.$info['name'].'" desc="'.$info['des'].'" max="'.$info['max_user'].'" audio="false" video="false" passallmessage="false" ><audio enable="0" /></Command>';
						  $result = "";
						  $resultDoc = "";
						  $fp = @fsockopen($host, $port, $errno, $errstr, 2);
						  if(!$fp)
