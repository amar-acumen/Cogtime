...
class Edit_video extends Admin_base_Controller
{
    ...
    function __construct()
    {
        try
        {
		   ...
		   $this->upload_path_featured_home = BASEPATH.'../uploads/media_center_video_featured_home/'; 
    ...
    public function index($current_page,$video_id) 
    {
        ...
        try
        {
            ...
            $data['contents'] = $this->landing_page_cms_model->get_contents_by_video_id($video_id);
            
            $this->session->set_userdata('current_page',$current_page);
            # rendering the view file...
            $VIEW_FILE = "admin/media_center/edit_video.phtml";
    ...
    function post_add_data()
    {
        ...
        $video_id = $this->input->post('i_video_id');
            ...
            else
            {
                                ...
                                try {
                                    $prev_video_img = $this->landing_page_cms_model->get_video_thumb_by_id($video_id);

                                    $img_path = BASEPATH."../uploads/media_center_videos/".getThumbName($prev_video_img,'bigthumb');
									$img_path = BASEPATH."../uploads/media_center_videos/".getThumbName($prev_video_img,'midthumb');
									$img_path = BASEPATH."../uploads/media_center_videos/".getThumbName($prev_video_img,'thumb');
            ...
            if($error!=1)  
            {
				...
				{
					$news_info = $this->landing_page_cms_model->get_contents_by_video_id($video_id);
					...
					unlink($this->upload_path. getThumbName($news_info['s_image'], 'sliderthumb'));
				}
				
				if($_FILES['adv_image_featured']['name']!='')
				{
					$news_info_featured = $this->landing_page_cms_model->get_contents_by_video_id($video_id);
				...
				$this->landing_page_cms_model->update_video_all($video_id);
				$res = $this->landing_page_cms_model->update_video($info,$video_id);
            }
