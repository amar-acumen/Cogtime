...
class Media_audio_category extends Admin_base_Controller
{
    ...
    public function index($page = 0) 
    {
        ...
        try
        {
            ...
            $data['categories'] = $this->landing_page_cms_model->fetch_all_audio_categories();
            ...
            ### ajax call ###
            ob_start();
            $this->audio_category_listing_ajax($current_page);
...
//echo "current page : ".$current_page.'--';
            ...
            $VIEW_FILE = "admin/media_center/media_audio_category.phtml";
    ...
    public function audio_category_listing_ajax($page=0)
    {
         try
         {
             //echo $page;
            ...
            $result = $this->landing_page_cms_model->fetch_all_audio_categories($s_where ,$page,$this->news_pagination_per_page,$order_by);
            // echo 'res :'.count($result);                                                   
            
            $resultCount = count($result);
            $total_rows = $this->landing_page_cms_model->get_total_audio_cat($s_where); 
            if(count($result)==0 && $total_rows)
            {
                $page= $page - $this->news_pagination_per_page;
                $result = $this->landing_page_cms_model->fetch_all_audio_categories($s_where ,$page,
    ...
    //echo "total : ".$total_rows;exit;
            ...
            $config['base_url'] = base_url()."admin/media_center/media_audio_category/audio_category_listing_ajax";
            ...
            # rendering the view file...
            $VIEW_FILE = "admin/media_center/media_audio_category_ajax.phtml";
    ...
    /***********************************************************************/
     function audio_status_disable($id){
        $id = $this->input->post('id');
        $data = array(
               'is_hidden' => 1,
...
$this->db->where('id', $id);
$this->db->update('cg_mc_audio_cat', $data); 
    }
    
     function audio_status_enable($id){
        $id = $this->input->post('id');
        $data = array(
               'is_hidden' => 0,
