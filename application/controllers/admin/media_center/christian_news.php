...
class Christian_news extends Admin_base_Controller
{
...
//echo "current page : ".$current_page.'--';
            ...
            $VIEW_FILE = "admin/media_center/christian_news.phtml";
    ...
    public function christian_news_listing_ajax($page=0)
    {
         try
         {
             //echo $page;
           ...
           /* if($_POST && $_POST['hd_val'] == 'Y')
			 ...
			 if($_POST)
            {
               ...
               // $posted_on = $this->input->post('create_time');
                if($title!='')
                {
                    $where.= " AND n.s_title LIKE '{$title}%'";
				...
				if($this->input->post('create_time') != ''){
					...
					$where .= ($dt_start_date=='')?'':" AND (DATE(n.dt_posted_on) ='".$dt_start_date."' )";
				}
				...
				if($this->input->post('featured')!='-1')
				{
					...
					$where.= " AND n.i_is_top_story=".$featured;
				}
            ...
            $result = $this->landing_page_cms_model->get_all_news($s_where ,$page,$this->news_pagination_per_page,$order_by);
            // echo 'res :'.count($result);                                                   
            
            $resultCount = count($result);
            $total_rows = $this->landing_page_cms_model->get_total_news($s_where); 
            if(count($result)==0 && $total_rows)
            {
                $page= $page - $this->news_pagination_per_page;
                $result = $this->landing_page_cms_model->get_all_news($s_where ,$page,
    ...
    //echo "total : ".$total_rows;exit;
            ...
            $config['base_url'] = base_url()."admin/media_center/christian_news/christian_news_listing_ajax";
            ...
            # rendering the view file...
            $VIEW_FILE = "admin/media_center/media_article_ajax.phtml";
...
$this->db->where('id', $id);
$this->db->update('cg_christian_news_cat', $data); 
    }
     function enable($id){
        $id = $this->input->post('id');
        $data = array(
