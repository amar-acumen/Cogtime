...
class Intercession extends Admin_base_Controller
{
   ...
   //echo "session : ".$this->session->userdata('search_condition');exit;
    ...
    public function all_pending_requests_ajax_pagination($page = 0) {
        try {
            $ring_id = $this->input->post('id');
            ...
            $result = get_all_ring_details_by_id($ring_id);
            //pr($result,1);
            ...
            # loading the view-part...
            $content = $this->load->view('admin/social_hub/rings/ring_pending_requests_ajax.phtml', $data, TRUE);
    ...
    public function all_commitments_ajax_pagination() {
        try {
            $e_id = $this->input->post('id');
            //$where = "WHERE A.i_ring_post_id=" . $article_id;
            //$result = $this->my_blog_post_model->show_all_comments($where,$page,$this->comments_pagination_per_page);
            $result = $this->intercession_model->get_commitments_by_id($e_id);
            //pr($res);
           ...
           // $data['no_of_result'] = $total_rows;
            ...
            # loading the view-part...
            $content = $this->load->view('admin/holy_place/intercession_commitment_ajax.phtml', $data, TRUE);
    ...
    public function delete_commitment()
	{
	 try {
            $id = $this->input->post('id');
            ...
            //$where = "WHERE A.i_ring_post_id=" . $article_id;
            //$result = $this->my_blog_post_model->show_all_comments($where,$page,$this->comments_pagination_per_page);
			$this->intercession_model->delete_commitment($id);
            $result = $this->intercession_model->get_commitments_by_id($e_id);
            //pr($res);
            // $total_rows=$this->ring_post_comments_model->get_by_ring_post_id($article_id);
            #Jquery Pagination Starts
           ...
           // $data['no_of_result'] = $total_rows;
            ...
            echo json_encode(array('html' => $content,'msg'=>'commitment has been deleted successfully.'));
        } catch (Exception $err_obj) {
            show_error($err_obj->getMessage());
        }
	}
