...
class Blogs extends Admin_base_Controller
{
    ...
    public function index() 
    {
        ...
        try
        {
            ...
            $VIEW_FILE = "admin/social_hub/blogs/blogs.phtml";
		  ...
		  try
		  {
			...
			if($_POST){
				  ...
				  if( count($arr_messages)==0 ) {
                         ...
                         $info['s_description'] = get_formatted_string($this->input->post('limitedtextarea'));    
						 $info['dt_updated_on'] = get_db_datetime();
						
						 $_ret = $this->my_blog_model->edit_blog($info, $id);
			  ...
			  }else
			  {
					...
					$data['posted'] = $this->my_blog_model->get_by_id($id) ;
					echo json_encode( array('result'=>'success',
                                            's_title'=>$data['posted']['s_title'],
                                            's_description'=>$data['posted']['s_description']) );	  
    ...
    {
		$i_ret=$this->my_blog_model->delete_by_id($id);
    ...
    public function ajax_pagination($page=0)
    {
        try
        {
			...
			$total_rows = $this->my_blog_model->get_list_count($s_where);
            ...
            $config['base_url'] = base_url()."admin/social_hub/blogs/ajax_pagination";
          ...
          echo  $this->load->view('admin/social_hub/blogs/list_blog_ajax.phtml', $data,TRUE);
		 }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
	 ...
	 public function change_status()
	 {
			...
			{
				$this->my_blog_model->change_status($i_status,$ID);
				if($i_status==1)
				   {
