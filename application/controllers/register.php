...
//				//$containsSpecial = preg_match('/[^a-zA-Z\d]/', $posted["txt_chat_display_name"]);
//				//$containsAll =$containsLetter+ $containsDigit;
//				if($containsDigit<1)
//				{
//
//				$data['error_chatname']="* Chat name must be between 1-6 letters and contain 1 digit";
//				}
//				}
                             /**************************************************************/   
...
//				}
			...
			$this->form_validation->set_rules('year', 'Birth date', 'trim|required'); 
               ...
               /* if ($posted["s_ssc"] == strtoupper($posted["s_security_code"])) {
                    //$this->form_validation->set_rules('security_code', 'Verification Code','trim|required|matches[ssc]');
                    $data['error_ssc'] = "* Captcha is Case Sensitive.";
                }*/
...
//exit;
	...
	echo '========'.$_SESSION['first_login'];exit;
        ...
        if ($info['i_status'] == 1) {
			...
			if ($info['is_first_login_checked'] == 1) {
