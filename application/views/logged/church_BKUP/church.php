...
//               $this->data["MAIN_MENU_SELECTED"] = 1;
        ...
        $data['member_arr'] =     $this->church_new_model->get_churchmembers($c_id);
 ...
 function add_church_data(){
     // pr($_POST,1);
    $info['ch_open_time'] = $this->input->post('ch_open');
    $info['ch_close_time'] = $this->input->post('ch_close');
    ...
    if($_FILES['ch_logo']['name']!='')
    {
        ...
        {
            $flag = 1;
            $_SESSION['ch_logo_msg'] = "Upload image size must be 260x40 ";
    ...
    if($_FILES['ch_cover']['name']!='')
    {
        ...
        {
            $flag = 1;
            $_SESSION['ch_cover_msg'] = "Upload image size must be 1300x400 ";
            header('location:church_setting');
            exit;
        }
    }
    ...
    {
        if($width_ch_profile_img > 135 && $height_ch_profile_img > 135)
        {
            $flag = 1;
            $_SESSION['ch_profile_img_msg'] = "Upload image size must be 136x136 ";
            header('location:church_setting');
            exit;
        }
    }
...
function general_setting(){
  //  pr($_POST,1);
    $wel_hed = $this->input->post('wel_hed');
    $wel_des = $this->input->post('wel_des');
    $ch_id = $this->input->post('ch_id');
    $data = array(
               'w_heading' => $wel_hed,
               'w_des' => $wel_des
...
//               $this->data["MAIN_MENU_SELECTED"] = 1;
        ...
        if(count($_POST)>0)
        {
            ...
            else
            {
                ...
                $data['member_arr'] =     $this->church_new_model->get_churchmembers($c_id,$wh);
            }
