	...
	public function get_player($width = '', $height = '', $autoplay=false) {
		...
		if( $this->video_site == 'youtube' ) {
			if( $width == '' || $height == '' ) {
				/*$width = '640';
				$height = '385';*/
                $width = $ci->config->item('webzine_main_video_width');
                $height = $ci->config->item('webzine_main_video_height');
			}
            ...
            //$html = '<iframe class="youtube-player" type="text/html" width="'. $width .'" height="'. $height .'" src="http://www.youtube.com/v/'. $this->video_id .'" frameborder="0"></iframe>';
