...
{
	$ci=& get_instance();
	 $sql="select * from ".$table." where ".$column."='".$window_id."' order by id DESC";        //exit();
	$res=$ci->db->query($sql);
	$s=$res->result_array();
...
{
	$ci=& get_instance();
	$query=$ci->db->query("select count(id) as ct from cg_church_member where church_id='".$cid."' and is_approved=1 and is_blocked = 1 and is_leave = 0");
	$result=$query->result_array();
	return $result['0']['ct'];
}
...
function is_friend($login_id, $profile_id) {
	...
	/*$s_qry = "SELECT   c.id, 
					...
					FROM 
						cg_user_contacts c ,cg_users u WHERE 
						1
						AND i_deleted_by = 1
