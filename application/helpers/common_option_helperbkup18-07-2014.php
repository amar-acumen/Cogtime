...
function makeOptionTimezones($s_id = '')
{
	$timezones = 
					  array (
...
### new month option  box for  paypal
 ...
 function makeNetpalLanguage($selectlang)
 {
    ...
    {
        $CI = & get_instance();
		$res = $CI->db->query("SELECT s_languages FROM {$CI->db->USERS} WHERE s_languages!='' AND i_isdeleted=1 AND i_status=1");
        $rstlang = $res->result_array();
    	
		foreach($rstlang as $v)
		{
			$lang	= explode(',',$v['s_languages']);
			foreach($lang as $val)
			{
