...
function delete_()
{
		...
		//alert(selected_id);
   	var URL = admin_base_url + 'trade_center/efreebie/delete/';
   $.ajax({
		type: 'POST',
		url:URL,
		dataType: 'json',
...
{
	var URL = admin_base_url +'trade_center/efreebie/change_status/';
	showBusyScreen();
	if(ajax_req!=null) {
		ajax_req.abort();
	}
...
function delete_req()
{
		...
		//alert(selected_id);
		var URL = admin_base_url + 'trade_center/efreebie/delete_request/';
		   $.ajax({
				type: 'POST',
				url:URL,
				dataType: 'json',
...
{
	var URL = admin_base_url +'trade_center/efreebie/change_request_status/';
	showBusyScreen();
	if(ajax_req!=null) {
		ajax_req.abort();
	}
...
function show_subcategory(p_id){
	  $.ajax({
		type: 'POST',
		url:admin_base_url +'trade_center/efreebie/generate_sub_categroies/',
		dataType: 'json',
		data: ({ 'id' : p_id
			 }),
		success: function (data, status) {
...
function get_details(pid){
	 ...
	 $.ajax({
		type: 'POST',
		url:admin_base_url +'trade_center/efreebie/get_details/',
		dataType: 'json',
		data: ({ 'id' : pid
			 }),
		success: function (data, status) {
...
$(document).ready(function(arg) {
 $('#frm_search').submit(function(){ 
 	var URL;
 		if($('#frm_typ').val() == 0)
		 URL = admin_base_url+"trade_center/efreebie/product_ajax_pagination/";
		else
		 URL = admin_base_url+"trade_center/efreebie/request_ajax_pagination/";
