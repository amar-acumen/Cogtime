// AJAX PHOTOT UPLOADER FOR WALL 
$(function()
{
	/**---------------- Multiple Upload -------------------------*/
	var btnUpload	= $('#btn_video_upload');
	var status	= $('#status');
					...
					onSubmit: 	function(file, ext)
							{
                                ...
                                errMsg.empty();
								status.html('<div class="info_msg">Loading ...</div>');
								showLoading();
							},
					onComplete: 	function(file, response)
							{
								...
								if(arr[0]==="ok")
								{
									...
									$('#all_videos').append($('<div id="div'+ nm_wo_dot +'" ><div class="view-thumb"  style="background:url('+ _temp_img +')  no-repeat center;max-width:60; height:60;"><a href="javascript:void(0);" class="remove" onclick="javascript:delvid(\''+ nm_w_sep +'\')"></a></div></div>'));
																		
									$('#'+ radio_fld_id).attr('checked', 'checked');
									$("#clr_div").remove();
									
									$('<div id="clr_div" class="clr"></div>').appendTo('#all_videos');
                                    
									$('<input type="hidden" id="hid_'+ radio_fld_id +'" name="video[]" />').appendTo('#hdnvideoflds').val(arr[2]);
									
									if( $('#all_videos').css('display')=='none' )
										$('#all_videos').show();
									//appbtnUpload.hide();
								}
								else
...
function delvid(filename)
{
	...
	$.get(delAJAXurl, function(msg){
		...
		if(msg === 'ok')
		{
			...
			status.html('<div class="ok_msg">Video deleted successfully ...</div>');
			btnuplad.show();
		}
		else
		{
			status.attr('class', 'ok');
			status.html('<div class="err_msg">Video cannot be deleted!</div>');
		}
	});
};
