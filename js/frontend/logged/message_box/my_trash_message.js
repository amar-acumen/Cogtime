...
function delete_messages() 
{
	...
	$(':checkbox:checked').each(function(i){
		...
		if($(this).attr('id').substr(0, 8) == 'chk_mail') {
			...
			type_vals  += $(this).attr('fld_typ')+',';
		}
	});
	csv_mail_ids = csv_mail_ids.substring(0, csv_mail_ids.length-1);
	
	//alert(csv_mail_ids);
	type_vals = type_vals.substring(0, type_vals.length-1);
	...
	ajax_req = $.ajax({
		type: 'POST',
		url:base_url+'logged/my_msg_outbox/delete_trash_messages/',
		dataType: 'json',
		data: ({'csv_mail_ids': csv_mail_ids ,'type_vals':type_vals,'current_page' : $('#current_page').val()}),
		success: function (data, status) {
			 hideBusyScreen();
			 showUIMsg(data.msg);
...
function move_messages() 
{
	...
	ajax_req = $.ajax({
		type: 'POST',
		url:base_url+'logged/my_msg_outbox/move_messages/',
		dataType: 'json',
		data: ({'csv_mail_ids': csv_mail_ids ,'type_vals':type_vals,'current_page' : $('#current_page').val()}),
		success: function (data, status) {
			 hideBusyScreen();
...
function move_checked_messages() { 
	var j = 0;
	$(':checkbox:checked').each(function(i){
		if($(this).attr('id').substr(0, 8) == 'chk_mail') {
			j++;
