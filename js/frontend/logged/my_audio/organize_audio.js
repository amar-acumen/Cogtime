function displayOrderAJAX(recordId, i_album_id, status)
{
	...
	var URL = base_url +'logged/organize_audio/maintain_displayorder_ajax';
...
function delete_()
{
	...
	if( selected_id!=0 )
	{
		showUIMsg("Audio deleted successfully.");
		var delURL = base_url + 'logged/my_audios/delete/'+ selected_id+'/'+album_id;
...
{ 
	...
	var URL = base_url +'logged/organize_audio/change_audio_album';
	showBusyScreen();
		...
		success: function (data, status) {
						...
						if(data.html != ''){
							...
							$('#album_div').html(data.html);
							 ...
							 $('#no_audio_box').remove();
							$('#album_div').append('<div id="no_audio_box" style="padding-top:5px;" class="section01"><div style="width:260px;" class="shade_norecords"><p class="blue_bold12">No Audios.</p></div></div>')
						 }
						if(data.view_more==true)
						{
							 $('#album_div').append('<div class="view_more" id="show_more_feeds_div"><a href="javascript:void(0);"id="show_more_feeds_link" page_value="'+data.current_page+'" onclick="show_more_feeds('+data.current_page+')">[view more] </a></div>');
