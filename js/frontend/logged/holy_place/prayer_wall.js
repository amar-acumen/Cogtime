...
function stop_edit_prayer_request() {
    show_dialog('edit-prayer');
}
...
function show_comment(p_request_id){
	
	$('#comment_form'+p_request_id).slideToggle('slow');
}

function add_comment(p_request_id) 
{
		
		var message  = $('#ta_comment'+p_request_id).val();
		...
		ajax_req = $.ajax({
			...
			url: base_url+'logged/prayer_wall/post_comment/'+ p_request_id,
			dataType: 'json',
			data: {'message':message},
			type: 'post',
			...
			success: function (data, status) {
				...
				if(data.success == false){
					...
					//alert(data.comm_count);
					$('#add_comment_frm'+p_request_id)[0].reset();	
		  			$('#ta_comment'+p_request_id).val('Max 500 Characters');
					
					$('#total_comments'+p_request_id).html('');
					$('#total_comments'+p_request_id).html('('+data.comm_count+')');
					
					showUIMsg(data.msg);
					$('#comment_form'+p_request_id).slideToggle('slow');
                    
                    
                  //  $('#add_comment_'+p_request_id).remove();
                   // $('#edit_prayer_req_'+p_request_id).addClass('first');
				   hideUILoader_nodialog();
...
function view_comments(id)
 {
	...
	$('#comment-container'+id).slideToggle('slow');
	$('#comment-container'+id).show();
	
	
	$.ajax({
		type : "post",
		url : base_url+"logged/prayer_wall/viewComments",
		dataType : "json",
		data : ({'p_request_id' : id }),
		success : function(data)
		{
			  ...
			  if(data.des != ''){
				$('#comment_content_'+id).html(base64_decode(data.des));
			  }else{
				  $('#comment_content_'+id).html("<div class ='view_more' style=' text-align: center; font-size: 15px; color:#013D62;'>No Comments.</div>");
