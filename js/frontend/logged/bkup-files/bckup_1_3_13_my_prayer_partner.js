...
/* Decline any friend request . */
 ...
 function if_any_exists_(id)
 {
	...
	$.ajax({
		...
		success : function(data)
		{
			...
			$('#text_area_prayer_point').val(data.des);
 ...
 function write_prayer_partner_()
 {
 	showUILoader_nodialog('<img src="'+base_url+'images/loading_big.gif" width="50"/> ');
 	//show_dialog('write_prayer_point');
	var id = $('#dbId').val();
	var des = $('#text_area_prayer_point').val();
 ...
 function read_prayer_partner_(id,mode)
 {
 	showUILoader_nodialog('<img src="'+base_url+'images/loading_big.gif" width="50"/> ');
	...
	$.ajax({
		...
		url : base_url+"logged/my_prayer_partners/read_prayer_partner",
		dataType : "json",
		data : ({'frnd_id' : id , 'mode': mode}),
		success : function(data)
		{
			
			$('#prayer_points').html(data.des);
