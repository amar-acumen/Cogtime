...
//// function to re-instantiate infinite-scrolling function...(for hompage)
function hp_infinite_scroller(selected_div)
{
	var nav_selector = 'div#page-nav-'+ selected_div;
	var selected_div_id = '#'+ selected_div;
	...
	$(selected_div_id).infinitescroll({
		...
		loading      : {
			...
			//alert("initialize success of hp util.js");
			
			if ($('span').hasClass('star-rating-control')) {
     			 
				 //// 1st, getting the selected div-id...
				
				 	//$('.playlist_star_rate13').rating();
					$('input[class^=playlist_star_rate]').rating();
					//$('input[id^=playlist_rating]').trigger('click');
					//alert("initialize success of hp util.js");
			}
...
//// function to show "add photo" dialog window...
var dialog_add_photo = null;
function show_add_photo_dialog()
{
	if(!dialog_add_photo) dialog_add_photo = null;
	dialog_add_photo = new ModalDialog ("#add_photo_short_cut");
	dialog_add_photo.show();
}