...
function ModalDialog (boxSpec) {
	...
	if (jQuery.browser.msie && (jQuery.browser.version < 7)) {
		...
		if(!jQuery.browser.msie)
			styleStr += "position:fixed; opacity: " + OVERLAY_OPACITY + ";" + sizeSpec; //styleStr += "position:fixed; opacity: " + OVERLAY_OPACITY + ";" + sizeSpec;
		else
    ...
    this.show =  function () {
		...
		if( NO_ZINDEX ) {
			box.css ({display: "block", top: top + "px",  left: (vp.x + (vp.w - box.width())/2) + "px"});
		}
		else {
