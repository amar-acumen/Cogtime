...
;(function ( $, window, document, undefined ) {
	$.widget("ui.triggeredAutocomplete", $.extend(true, {}, $.ui.autocomplete.prototype, {
		...
		_renderItem:  function( ul, item ) {// console.log(ul);
		
		 	var ul_elem = ul;
		
			if(item.img != undefined) {
				//$('ul.ui-autocomplete').attr('style','left: 373px; width: 456px; display: block; top: 636px; z-index: 1000;');
		...
		search: function(value, event) {
			...
			if (contents.indexOf(this.options.trigger) >= 0 && check_contents.match(regex)) {
				...
				var term = contents;//contents.substring(contents.lastIndexOf(this.options.trigger) + 1, contents.length);
